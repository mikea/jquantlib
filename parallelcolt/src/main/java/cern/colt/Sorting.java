...
public class Sorting extends Object {
	 ...
	 * @see Comparator
	 */
	public static void quickSortMulti(int[] a, int fromIndex, int toIndex, IntComparator c) {
		rangeCheck(a.length, fromIndex, toIndex);
		if ((Utils.getNP() > 1) && (a.length >= Utils.getThreadsBeginN_1D())) {
			QuickSortMulti.qsort(a, fromIndex, toIndex - fromIndex, c, 2 * Utils.getNP());
		} else {
			quickSort1(a, fromIndex, toIndex - fromIndex, c);
		}
	}
