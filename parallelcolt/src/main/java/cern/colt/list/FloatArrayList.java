...
/**
 * Resizable list holding <code>float</code> elements; implemented with
...
public class FloatArrayList extends AbstractFloatList {
	...
	public boolean removeAll(AbstractFloatList other) {
		...
		if ((N + M) * cern.jet.math.FloatArithmetic.log2(N) < M * N) {
	...
	public void shuffleFromTo(int from, int to) {
		...
		cern.jet.random.FloatUniform gen = new cern.jet.random.FloatUniform(new cern.jet.random.engine.FRand(new java.util.Date()));
		float tmpElement;
		float[] theElements = elements;
		int random;
		for (int i = from; i < to; i++) {
