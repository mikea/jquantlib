...
public abstract class FComplexMatrix1D extends AbstractMatrix1D {
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public FComplexMatrix1D assign(final cern.colt.function.FComplexFComplexFunction f) {
		int size = size();
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public FComplexMatrix1D assign(final cern.colt.function.FComplexProcedure cond, final cern.colt.function.FComplexFComplexFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public FComplexMatrix1D assign(final cern.colt.function.FComplexRealFunction f) {
		int size = size();
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public FComplexMatrix1D assign(final FComplexMatrix1D y, final cern.colt.function.FComplexFComplexFComplexFunction f) {
		int size = size();
		checkSize(y);
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
	 ...
	 * receiver. Complex data is represented by 2 float values in sequence: the
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.FloatProperty.DEFAULT.equals(this, (FComplexMatrix1D) obj);
	}
	 ...
	 * @return the new view.
	 */
	public FComplexMatrix1D viewSelection(cern.colt.function.FComplexProcedure condition) {
		int size = size();
