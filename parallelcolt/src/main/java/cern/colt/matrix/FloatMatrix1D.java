...
public abstract class FloatMatrix1D extends AbstractMatrix1D {
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public float aggregate(final FloatMatrix1D other, final cern.colt.function.FloatFloatFunction aggr, final cern.colt.function.FloatFloatFunction f) {
		checkSize(other);
		if (size == 0)
			return Float.NaN;
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public FloatMatrix1D assign(final cern.colt.function.FloatFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public FloatMatrix1D assign(final cern.colt.function.FloatProcedure cond, final cern.colt.function.FloatFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	 ...
	 * 
	 */
	public FloatMatrix1D assign(final cern.colt.function.FloatProcedure cond, final float value) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public FloatMatrix1D assign(final FloatMatrix1D y, final cern.colt.function.FloatFloatFunction function) {
		checkSize(y);
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public FloatMatrix1D assign(FloatMatrix1D y, cern.colt.function.FloatFloatFunction function, cern.colt.list.IntArrayList nonZeroIndexes) {
		checkSize(y);
		...
		} else if (function instanceof cern.jet.math.FloatPlusMult) {
			float multiplicator = ((cern.jet.math.FloatPlusMult) function).multiplicator;
	...
	/**
	 * Computes the discrete cosine transform (DCT-II) of this matrix. Throws
	...
	/**
	 * Computes the discrete sine transform (DST-II) of this matrix. Throws
	 * IllegalArgumentException if the size of this matrix is not a power of 2
	 * number.
	 */
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.FloatProperty.DEFAULT.equals(this, (FloatMatrix1D) obj);
	}
	 ...
	 * Throws IllegalArgumentException if the size of this matrix is not a power
	 * of 2 number.
	 */
	 ...
	 * instance of type <tt>DenseFloatMatrix1D</tt> the new matrix must also
	 * be of type <tt>DenseFloatMatrix1D</tt>, if the receiver is an instance
	 * of type <tt>SparseFloatMatrix1D</tt> the new matrix must also be of
	 * type <tt>SparseFloatMatrix1D</tt>, etc. In general, the new matrix
	 ...
	 * @return new 2D matrix with columns being the elements of this matrix.
	 */
	public abstract FloatMatrix3D reshape(int slices, int rows, int cols);
	 ...
	 * @return the new view.
	 */
	public FloatMatrix1D viewSelection(cern.colt.function.FloatProcedure condition) {
	 ...
	 * information, see
	 * {@link cern.colt.matrix.floatalgo.Sorting#sort(FloatMatrix1D)}. For more
	...
	public FloatMatrix1D viewSorted() {
		return cern.colt.matrix.floatalgo.Sorting.mergeSort.sort(this);
	}
	 ...
	 * <tt>Sum(x[i]*y[i])</tt>. Where <tt>x == this</tt>. Operates on
	 * cells at indexes <tt>0 .. Math.min(size(),y.size())</tt>.
	 * 
	 * @param y
	 *            the second vector.
	 ...
	 * Returns the dot product of two vectors x and y, which is
	 * <tt>Sum(x[i]*y[i])</tt>. Where <tt>x == this</tt>. Operates on
	 * cells at indexes <tt>from .. Min(size(),y.size(),from+length)-1</tt>.
	 * 
	 * @param y
	 *            the second vector.
	 * @param from
