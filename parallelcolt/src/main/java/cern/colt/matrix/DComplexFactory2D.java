...
import cern.colt.matrix.impl.SparseDComplexMatrix2D;
 ...
 * <td>Use methods
 * {@link #appendColumns(DComplexMatrix2D,DComplexMatrix2D) appendColumns},
 * {@link #appendColumns(DComplexMatrix2D,DComplexMatrix2D) appendRows} and
 * {@link  #repeat(DComplexMatrix2D,int,int) repeat} to append rows and columns.
 * </td>
 * </tr>
 * <tr align="left" valign="top">
 ...
 * {@link #sample(int,int,double[],double)  sample} to construct random
 * matrices. </td>
 * </tr>
 * </table>
 * <p>
...
public class DComplexFactory2D extends cern.colt.PersistentObject {
	...
	public DComplexMatrix2D random(int rows, int columns) {
		return make(rows, columns).assign(cern.jet.math.DComplexFunctions.random());
	}
	...
	public DComplexMatrix2D sample(DComplexMatrix2D matrix, double[] value, double nonZeroFraction) {
		...
		cern.jet.random.sampling.DoubleRandomSamplingAssistant sampler = new cern.jet.random.sampling.DoubleRandomSamplingAssistant(n, size, new cern.jet.random.engine.DoubleMersenneTwister());
		for (int i = 0; i < size; i++) {
