...
public abstract class ObjectMatrix3D extends AbstractMatrix3D {
	 ...
	 * information, see
	 * {@link cern.colt.matrix.objectalgo.Sorting#sort(ObjectMatrix3D,int,int)}.
	 ...
	 * 	for (int row = rows; --row &gt;= 0;) {
	 * 		for (int column = columns; --column &gt;= 0;) {
	 * 			if (!procedure.apply(get(slice, row, column)))
	 * 				return false;
	 * 		}
	 * 	}
	 * }
	 ...
	 * 
	 * @param procedure
	 *            a procedure object taking as argument the current cell's
	 *            value. Stops iteration if the procedure returns <tt>false</tt>,
	 *            otherwise continues.
	 ...
	 * 
	 * @param procedure
	 *            a procedure object taking as first argument the current slice,
	 *            as second argument the current row, and as third argument the
	 *            current column. Stops iteration if the procedure returns
