...
import cern.colt.matrix.impl.DenseDComplexMatrix2D;
...
public abstract class DComplexMatrix2D extends AbstractMatrix2D {
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public double[] aggregate(final DComplexMatrix2D other, final cern.colt.function.DComplexDComplexDComplexFunction aggr, final cern.colt.function.DComplexDComplexDComplexFunction f) {
		checkShape(other);
		double[] b = new double[2];
		if (size() == 0) {
			b[0] = Double.NaN;
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public DComplexMatrix2D assign(final cern.colt.function.DComplexDComplexFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public DComplexMatrix2D assign(final cern.colt.function.DComplexProcedure cond, final cern.colt.function.DComplexDComplexFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	 ...
	 * 
	 */
	public DComplexMatrix2D assign(final cern.colt.function.DComplexProcedure cond, final double[] value) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public DComplexMatrix2D assign(final cern.colt.function.DComplexRealFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public DComplexMatrix2D assign(final DComplexMatrix2D y, final cern.colt.function.DComplexDComplexDComplexFunction f) {
		checkShape(y);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.DoubleProperty.DEFAULT.equals(this, (DComplexMatrix2D) obj);
	}
	 ...
	 * @return <tt>this</tt> (for convenience only).
	 */
	public DComplexMatrix2D forEachNonZero(final cern.colt.function.IntIntDComplexFunction function) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
				...
				futures[j] = Utils.threadPool.submit(new Runnable() {
					...
					public void run() {
						for (int r = startrow; r < stoprow; r++) {
							...
							zLoc.setQuick(r, DComplex.plus(DComplex.mult(s, alpha), DComplex.mult(zLoc.getQuick(r), beta)));
						}
					}
				});
			}
	...
	public DComplexMatrix2D zMult(DComplexMatrix2D B, DComplexMatrix2D C) {
		return zMult(B, C, new double[] { 1, 0 }, (C == null ? new double[] { 1, 0 } : new double[] { 0, 0 }), false, false);
	}
	...
	public DComplexMatrix2D zMult(final DComplexMatrix2D B, DComplexMatrix2D C, final double[] alpha, final double[] beta, boolean transposeA, boolean transposeB) {
		...
		final DComplexMatrix2D C_loc;
		if (C == null) {
			C_loc = new DenseDComplexMatrix2D(m, p);
		} else {
			C_loc = C;
		}
		if (B.rows != n)
					...
					public void run() {
						for (int a = starta; a < stopa; a++) {
							for (int b = 0; b < m; b++) {
								...
								C_loc.setQuick(b, a, DComplex.plus(DComplex.mult(s, alpha), DComplex.mult(C_loc.getQuick(b, a), beta)));
							}
						}
					}
				});
	...
	public double[] zSum() {
		...
		return aggregate(cern.jet.math.DComplexFunctions.plus, cern.jet.math.DComplexFunctions.identity);
	}
