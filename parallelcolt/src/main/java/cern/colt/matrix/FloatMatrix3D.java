...
public abstract class FloatMatrix3D extends AbstractMatrix3D {
	 ...
	 * terminators are <tt>a(1) == f(get(0,0,0)), a(0)==Float.NaN</tt>.
	 * <p>
	 * <b>Example:</b>
	 * 
	 * <pre>
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = Utils.threadPool.submit(new Callable<Float>() {
					...
					public Float call() throws Exception {
						float a = f.apply(getQuick(startslice, 0, 0));
						int d = 1;
	...
	public float aggregate(final FloatMatrix3D other, final cern.colt.function.FloatFloatFunction aggr, final cern.colt.function.FloatFloatFunction f) {
		...
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_3D())) {
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = Utils.threadPool.submit(new Callable<Float>() {
					public Float call() throws Exception {
						float a = f.apply(getQuick(startslice, 0, 0), other.getQuick(startslice, 0, 0));
						int d = 1;
						for (int s = startslice; s < stopslice; s++) {
							for (int r = 0; r < rows; r++) {
								for (int c = d; c < columns; c++) {
		...
		if ((np > 1) && (slices * rows * columns >= Utils.getThreadsBeginN_3D())) {
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = Utils.threadPool.submit(new Callable<Float>() {
					...
					public Float call() throws Exception {
						float a = f.apply(getQuick(indexesArray[startidx][0], indexesArray[startidx][1], indexesArray[startidx][2]));
						float elem;
						for (int r = startidx + 1; r < stopidx; r++) {
							elem = getQuick(indexesArray[r][0], indexesArray[r][1], indexesArray[r][2]);
			...
			float elem;
			a = f.apply(getQuick(indexesArray[0][0], indexesArray[0][1], indexesArray[0][2]));
			for (int r = 1; r < n; r++) {
				elem = getQuick(indexesArray[r][0], indexesArray[r][1], indexesArray[r][2]);
				a = aggr.apply(a, f.apply(elem));
			}
	 ...
	 * @see cern.jet.math.FloatFunctions
	 */
	public FloatMatrix3D assign(final FloatMatrix3D y, final cern.colt.function.FloatFloatFunction function, Set<int[]> indexes) {
		checkShape(y);
		int n = indexes.size();
		final int[][] indexesArray = indexes.toArray(new int[0][0]);
		int np = Utils.getNP();
		if ((np > 1) && (rows * columns >= Utils.getThreadsBeginN_3D())) {
			Future[] futures = new Future[np];
			int k = n / np;
			for (int j = 0; j < np; j++) {
				final int startidx = j * k;
				...
				futures[j] = Utils.threadPool.submit(new Runnable() {
					...
					public void run() {
						for (int r = startidx; r < stopidx; r++) {
							setQuick(indexesArray[r][0], indexesArray[r][1], indexesArray[r][2], function.apply(getQuick(indexesArray[r][0], indexesArray[r][1], indexesArray[r][2]), y.getQuick(indexesArray[r][0], indexesArray[r][1], indexesArray[r][2])));
						}
					}
	 ...
	 * matrix. Throws IllegalArgumentException if the column size or the row
	...
	/**
	 * Computes the 3D discrete cosine transform (DCT-II) of this matrix. Throws
	...
	/**
	 * Computes the 3D discrete sine transform (DST-II) of this matrix. Throws
	 * IllegalArgumentException if the slice size or the row size or the column
	 * size of this matrix is not a power of 2 number.
	 * 
	 * @param scale
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.FloatProperty.DEFAULT.equals(this, (FloatMatrix3D) obj);
	}
	 ...
	 * (DFT) of each slice of this matrix. Throws IllegalArgumentException if
	 * the column size or the row size of a slice is not a power of 2 number.
	 * 
	 * @return the 2D discrete Fourier transform (DFT) of each slice of this
	 *         matrix.
	 */
	public abstract FComplexMatrix3D getFft2Slices();

	/**
	 * Returns new complex matrix which is the 3D discrete Fourier transform
	 * (DFT) of this matrix. Throws IllegalArgumentException if the slice size
	 * or the row size or the column size of this matrix is not a power of 2
	 * number.
	 * 
	 * @return the 3D discrete Fourier transform (DFT) of this matrix.
	 */
	public abstract FComplexMatrix3D getFft3();

	/**
	 * Returns new complex matrix which is the 2D inverse of the discrete
	 * Fourier transform (IDFT) of each slice of this matrix. Throws
	 * IllegalArgumentException if the column size or the row size of a slice is
	 * not a power of 2 number.
	 * 
	 * @return the 2D inverse of the discrete Fourier transform (IDFT) of each
	 *         slice of this matrix.
	 */
	public abstract FComplexMatrix3D getIfft2Slices(boolean scale);
	 ...
	 * if the slice size or the row size or the column size of this matrix is
	 * not a power of 2 number.
	 * 
	 * @return the 3D inverse of the discrete Fourier transform (IDFT) of this
	 *         matrix.
	 */
	public abstract FComplexMatrix3D getIfft3(boolean scale);
	 ...
	 * 
	 * @return a HashMap with keys being the locations ({slice_location,
	 *         row_location, column_location}) and values being the negative
	 ...
	 * see
	 * {@link FloatMatrix2D#getNonZeros(IntArrayList,IntArrayList,FloatArrayList)}.
	 * 
	 * @param sliceList
	 ...
	 * each slice of this matrix. Throws IllegalArgumentException if the column
	 * size or the row size of a slice is not a power of 2 number.
	 * 
	 * @param scale
	 *            if true then scaling is performed
	 * 
	 ...
	 * this matrix. Throws IllegalArgumentException if the slice size or the row
	 * size or the column size of this matrix is not a power of 2 number.
	 * 
	 * @param scale
	 *            if true then scaling is performed
	 * 
	 ...
	 * matrix. Throws IllegalArgumentException if the slice size or the row size
	 ...
	 * information, see
	 * {@link cern.colt.matrix.floatalgo.Sorting#sort(FloatMatrix3D,int,int)}.
	...
	public FloatMatrix3D viewSorted(int row, int column) {
		return cern.colt.matrix.floatalgo.Sorting.mergeSort.sort(this, row, column);
	}
	 ...
	 * float apply( &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a000, float a001,
	 * float a002, &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a010, float a011,
	 * float a012, &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a020, float a021,
	 * float a022,
	 * 
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a100, float a101, float a102,
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a110, float a111, float a112,
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a120, float a121, float a122,
	 * 
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a200, float a201, float a202,
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a210, float a211, float a212,
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;float a220, float a221, float a222) {
	 * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return beta*a111 +
	 * alpha*(a000 + ... + a222); &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;} };
	 ...
	 * instance of type <tt>DenseFloatMatrix3D</tt> the new matrix must also
	 * be of type <tt>DenseFloatMatrix2D</tt>, if the receiver is an instance
	 * of type <tt>SparseFloatMatrix3D</tt> the new matrix must also be of
	 ...
	 */
	protected abstract FloatMatrix2D like2D(int rows, int columns, int rowZero, int columnZero, int rowStride, int columnStride);
