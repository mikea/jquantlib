...
public abstract class DoubleMatrix3D extends AbstractMatrix3D {
		...
		if ((np > 1) && (slices * rows * columns >= Utils.getThreadsBeginN_3D())) {
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = Utils.threadPool.submit(new Callable<Double>() {
					...
					public Double call() throws Exception {
						double a = f.apply(getQuick(indexesArray[startidx][0], indexesArray[startidx][1], indexesArray[startidx][2]));
						double elem;
	 ...
	 * @see cern.jet.math.DoubleFunctions
	 */
	public DoubleMatrix3D assign(final DoubleMatrix3D y, final cern.colt.function.DoubleDoubleFunction function, Set<int[]> indexes) {
		checkShape(y);
		int n = indexes.size();
		final int[][] indexesArray = indexes.toArray(new int[0][0]);
		int np = Utils.getNP();
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.DoubleProperty.DEFAULT.equals(this, (DoubleMatrix3D) obj);
	}
	 ...
	 * see
	 * {@link DoubleMatrix2D#getNonZeros(IntArrayList,IntArrayList,DoubleArrayList)}.
	 * 
	 * @param sliceList
	 ...
	 * information, see
	 * {@link cern.colt.matrix.doublealgo.Sorting#sort(DoubleMatrix3D,int,int)}.
	 ...
	 * instance of type <tt>DenseDoubleMatrix3D</tt> the new matrix must also
	 * be of type <tt>DenseDoubleMatrix2D</tt>, if the receiver is an
	 * instance of type <tt>SparseDoubleMatrix3D</tt> the new matrix must also
	 * be of type <tt>SparseDoubleMatrix2D</tt>, etc.
	 * 
	 * @param rows
	 *            the number of rows the matrix shall have.
	 * @param columns
