...
public abstract class FComplexMatrix3D extends AbstractMatrix3D {
	...
	public FComplexMatrix3D assign(FComplexMatrix3D other) {
		if (other == this)
			return this;
		checkShape(other);
		final FComplexMatrix3D other_loc;
				...
				futures[j] = Utils.threadPool.submit(new Runnable() {
					public void run() {
						for (int s = startslice; s < stopslice; s++) {
							for (int r = 0; r < rows; r++) {
								for (int c = 0; c < columns; c++) {
									setQuick(s, r, c, other_loc.getQuick(s, r, c));
								}
							}
						}
					}
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.FloatProperty.DEFAULT.equals(this, (FComplexMatrix3D) obj);
	}
	 ...
	 */
	public abstract void fft2Slices();
	 ...
	 */
	public abstract void ifft2Slices(boolean scale);
	 ...
	 */
	protected abstract FComplexMatrix2D like2D(int rows, int columns, int rowZero, int columnZero, int rowStride, int columnStride);
