...
import hep.aida.bin.DynamicFloatBin1D;
import cern.colt.function.FloatFloatFunction;
 ...
 * {@link hep.aida.bin}, in particular {@link hep.aida.bin.DynamicFloatBin1D}.
 * <p>
...
public class Statistic extends Object {
	private static final cern.jet.math.FloatFunctions F = cern.jet.math.FloatFunctions.functions;
	...
	public interface VectorVectorFunction {
		 ...
		 */
		abstract public float apply(cern.colt.matrix.FloatMatrix1D x, cern.colt.matrix.FloatMatrix1D y);
	}
	 ...
	 * @see Formatter
	 * @see hep.aida.bin.FloatBinFunction1D
	 * @see hep.aida.bin.FloatBinFunctions1D
	 */
	public static FloatMatrix2D aggregate(FloatMatrix2D matrix, hep.aida.bin.FloatBinFunction1D[] aggr, FloatMatrix2D result) {
		DynamicFloatBin1D bin = new DynamicFloatBin1D();
		float[] elements = new float[matrix.rows()];
		cern.colt.list.FloatArrayList values = new cern.colt.list.FloatArrayList(elements);
	...
	public static FloatMatrix2D covariance(FloatMatrix2D matrix) {
		...
		FloatMatrix2D covariance = new cern.colt.matrix.impl.DenseFloatMatrix2D(columns, columns);
	 ...
	 */
	public static hep.aida.FloatIHistogram2D cube(FloatMatrix1D x, FloatMatrix1D y, FloatMatrix1D weights) {
		if (x.size() != y.size() || y.size() != weights.size())
			throw new IllegalArgumentException("vectors must have same size");

		float epsilon = 1.0E-8f;
		cern.colt.list.FloatArrayList distinct = new cern.colt.list.FloatArrayList();
		float[] vals = new float[x.size()];
		cern.colt.list.FloatArrayList sorted = new cern.colt.list.FloatArrayList(vals);

		// compute distinct values of x
		x.toArray(vals); // copy x into vals
		sorted.sort();
		cern.jet.stat.FloatDescriptive.frequencies(sorted, distinct, null);
		...
		distinct.trimToSize();
		hep.aida.FloatIAxis xaxis = new hep.aida.ref.FloatVariableAxis(distinct.elements());
		...
		if (distinct.size() > 0)
			distinct.add(distinct.get(distinct.size() - 1) + epsilon);
		distinct.trimToSize();
		hep.aida.FloatIAxis yaxis = new hep.aida.ref.FloatVariableAxis(distinct.elements());

		hep.aida.FloatIHistogram2D histo = new hep.aida.ref.FloatHistogram2D("Cube", xaxis, yaxis);
	 ...
	 */
	public static hep.aida.FloatIHistogram3D cube(FloatMatrix1D x, FloatMatrix1D y, FloatMatrix1D z, FloatMatrix1D weights) {
		...
		if (distinct.size() > 0)
			distinct.add(distinct.get(distinct.size() - 1) + epsilon);
		distinct.trimToSize();
		hep.aida.FloatIAxis zaxis = new hep.aida.ref.FloatVariableAxis(distinct.elements());

		hep.aida.FloatIHistogram3D histo = new hep.aida.ref.FloatHistogram3D("Cube", xaxis, yaxis, zaxis);
	...
	public static FloatMatrix2D distance(FloatMatrix2D matrix, VectorVectorFunction distanceFunction) {
		int columns = matrix.columns();
		FloatMatrix2D distance = new cern.colt.matrix.impl.DenseFloatMatrix2D(columns, columns);
	 ...
	 */
	public static hep.aida.FloatIHistogram1D histogram(hep.aida.FloatIHistogram1D histo, FloatMatrix1D vector) {
	 ...
	 */
	public static hep.aida.FloatIHistogram2D histogram(hep.aida.FloatIHistogram2D histo, FloatMatrix1D x, FloatMatrix1D y) {
		if (x.size() != y.size())
			throw new IllegalArgumentException("vectors must have same size");
	 ...
	 *             <tt>x.size() != y.size() || y.size() != weights.size()</tt>.
	 */
	public static hep.aida.FloatIHistogram2D histogram(hep.aida.FloatIHistogram2D histo, FloatMatrix1D x, FloatMatrix1D y, FloatMatrix1D weights) {
		if (x.size() != y.size() || y.size() != weights.size())
			throw new IllegalArgumentException("vectors must have same size");
		for (int i = x.size(); --i >= 0;) {
	 ...
	 *             <tt>x.size() != y.size() || x.size() != z.size() || x.size() != weights.size()</tt>.
	 */
	public static hep.aida.FloatIHistogram3D histogram(hep.aida.FloatIHistogram3D histo, FloatMatrix1D x, FloatMatrix1D y, FloatMatrix1D z, FloatMatrix1D weights) {
		if (x.size() != y.size() || x.size() != z.size() || x.size() != weights.size())
			throw new IllegalArgumentException("vectors must have same size");
		for (int i = x.size(); --i >= 0;) {
	 ...
	 * @see cern.jet.random.sampling.FloatRandomSampler
	 */
	public static FloatMatrix1D viewSample(FloatMatrix1D matrix, float fraction, FloatRandomEngine randomGenerator) {
		...
		if (randomGenerator == null)
			randomGenerator = new cern.jet.random.engine.FloatMersenneTwister((int) System.currentTimeMillis());
		...
		int n = ncols;
		int N = matrix.size();
		cern.jet.random.sampling.FloatRandomSampler.sample(n, N, n, 0, selected, 0, randomGenerator);
