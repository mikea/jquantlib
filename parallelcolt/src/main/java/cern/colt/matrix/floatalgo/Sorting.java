...
public class Sorting extends cern.colt.PersistentObject {
	...
	protected void runSortMulti(int[] a, int fromIndex, int toIndex, IntComparator c) {
		cern.colt.Sorting.quickSortMulti(a, fromIndex, toIndex, c);
	}
	...
	/**
	 * Multithreaded method that sorts indices of the <code>vector</code> into
	 * ascending order.
	 * 
	 * @param vector
	 * @return sorted indices
	 */
	public int[] sortIndexMulti(final FloatMatrix1D vector) {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
		...
		runSortMulti(indexes, 0, indexes.length, comp);
	 ...
	 * @return sorted indices
	 */
	public int[] sortIndexMulti(final FloatMatrix1D vector, IntComparator comp) {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
	 ...
	 */
	public FloatMatrix1D sort(final FloatMatrix1D vector, final cern.colt.function.FloatComparator c) {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
	 ...
	 * @return sorted indices
	 */
	public int[] sortIndex(final FloatMatrix1D vector, final cern.colt.function.FloatComparator c) {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
	...
	/**
	 * Multithreaded method that sorts indices of the <code>vector</code>
	 * according to the comparator <code>c</code>.
	 * 
	 * @param vector
	 * @param c
	 * @return sorted indices
	 ...
	 */
	public int[] sortIndexMulti(final FloatMatrix1D vector, final cern.colt.function.FloatComparator c) throws IllegalAccessException {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
		IntComparator comp;
		if (!vector.isView()) {
			final float[] values = (float[]) vector.getElements();
			comp = new IntComparator() {
				public int compare(int a, int b) {
					return c.compare(values[a], values[b]);
				}
			};
		} else {
			comp = new IntComparator() {
	 ...
	 *         matrix is left unaffected.</b>
	 */
	public FloatMatrix2D sort(FloatMatrix2D matrix, hep.aida.bin.FloatBinFunction1D aggregate) {
		...
		hep.aida.bin.FloatBinFunction1D[] func = { aggregate };
	...
	public static void zdemo3() {
		...
		cern.colt.function.FloatComparator comp = new cern.colt.function.FloatComparator() {
			public int compare(float a, float b) {
		...
		sorted.assign(cern.jet.math.FloatFunctions.sin);
		/*
	...
	protected static void zdemo4() {
		...
		matrix1.assign(matrix2, cern.jet.math.FloatFunctions.pow);
	...
	public static void zdemo5(int rows, int columns, boolean print) {
		...
		final cern.jet.math.FloatFunctions F = cern.jet.math.FloatFunctions.functions;
		FloatMatrix2D A = cern.colt.matrix.FloatFactory2D.dense.make(rows, columns);
		A.assign(new cern.jet.random.engine.FRand()); // initialize randomly
		timer.stop().display();
		...
		timer.reset().start();
		// THE QUICK VERSION (takes some 10 secs)
		A = sort.sort(A, hep.aida.bin.FloatBinFunctions1D.median);
		...
		if (print) {
			...
			hep.aida.bin.FloatBinFunction1D[] funs = { hep.aida.bin.FloatBinFunctions1D.median, hep.aida.bin.FloatBinFunctions1D.sumLog, hep.aida.bin.FloatBinFunctions1D.geometricMean };
			...
			System.out.println("first part of sorted result = \n" + new cern.colt.matrix.floatalgo.Formatter("%G").toTitleString(A.viewPart(0, 0, r, columns), rowNames, columnNames, null, null, null, funs));
		}

		System.out.print("now sorting - slow version... ");
		A = B;
		cern.colt.matrix.floatalgo.FloatMatrix1DComparator fun = new cern.colt.matrix.floatalgo.FloatMatrix1DComparator() {
			public int compare(FloatMatrix1D x, FloatMatrix1D y) {
				float a = cern.colt.matrix.floatalgo.Statistic.bin(x).median();
				float b = cern.colt.matrix.floatalgo.Statistic.bin(y).median();
				// float a = x.aggregate(F.plus,F.log);
