...
public abstract class DoubleMatrix1D extends AbstractMatrix1D {
	 ...
	 * @see cern.jet.math.DoubleFunctions
	 */
	public double aggregate(final DoubleMatrix1D other, final cern.colt.function.DoubleDoubleFunction aggr, final cern.colt.function.DoubleDoubleFunction f) {
		checkSize(other);
		if (size == 0)
			return Double.NaN;
	 ...
	 * @see cern.jet.math.DoubleFunctions
	 */
	public DoubleMatrix1D assign(final cern.colt.function.DoubleFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	 ...
	 * @see cern.jet.math.DoubleFunctions
	 */
	public DoubleMatrix1D assign(final cern.colt.function.DoubleProcedure cond, final cern.colt.function.DoubleFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	 ...
	 * 
	 */
	public DoubleMatrix1D assign(final cern.colt.function.DoubleProcedure cond, final double value) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	 ...
	 * @see cern.jet.math.DoubleFunctions
	 */
	public DoubleMatrix1D assign(final DoubleMatrix1D y, final cern.colt.function.DoubleDoubleFunction function) {
		checkSize(y);
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
	...
	public DoubleMatrix1D assign(DoubleMatrix1D y, cern.colt.function.DoubleDoubleFunction function, cern.colt.list.IntArrayList nonZeroIndexes) {
		...
		if (function == cern.jet.math.DoubleFunctions.mult) { // x[i] = x[i] *
																// y[i]
			int j = 0;
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.DoubleProperty.DEFAULT.equals(this, (DoubleMatrix1D) obj);
	}
	 ...
	 * instance of type <tt>DenseDoubleMatrix1D</tt> the new matrix must also
	 * be of type <tt>DenseDoubleMatrix1D</tt>, if the receiver is an
	 ...
	 * information, see
	 * {@link cern.colt.matrix.doublealgo.Sorting#sort(DoubleMatrix1D)}. For
