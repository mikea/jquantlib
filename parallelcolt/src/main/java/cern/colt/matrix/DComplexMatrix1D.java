...
public abstract class DComplexMatrix1D extends AbstractMatrix1D {
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public DComplexMatrix1D assign(final cern.colt.function.DComplexDComplexFunction f) {
		int size = size();
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public DComplexMatrix1D assign(final cern.colt.function.DComplexProcedure cond, final cern.colt.function.DComplexDComplexFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public DComplexMatrix1D assign(final cern.colt.function.DComplexRealFunction f) {
		int size = size();
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
	 ...
	 * @see cern.jet.math.DComplexFunctions
	 */
	public DComplexMatrix1D assign(final DComplexMatrix1D y, final cern.colt.function.DComplexDComplexDComplexFunction f) {
		int size = size();
		checkSize(y);
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.DoubleProperty.DEFAULT.equals(this, (DComplexMatrix1D) obj);
	}
	 ...
	 */
	public abstract DComplexMatrix2D reshape(int rows, int cols);
	 ...
	 * @return the new view.
	 */
	public DComplexMatrix1D viewSelection(cern.colt.function.DComplexProcedure condition) {
		int size = size();
				...
				futures[j] = Utils.threadPool.submit(new Callable<double[]>() {
					public double[] call() throws Exception {
						...
						for (int k = startlength; k < stoplength; k++) {
							...
							sum = DComplex.plus(sum, DComplex.mult(getQuick(idx), tmp));
						}
						return sum;
					}
				});
	...
	public double[] zDotProduct(DComplexMatrix1D y, int from, int length, IntArrayList nonZeroIndexes) {
		...
		while ((--length >= 0) && (index < s) && ((i = nonZeroIndexElements[index]) < tail)) {
			tmp = y.getQuick(i);
			tmp[1] = -tmp[1]; // complex conjugate
			sum = DComplex.plus(sum, DComplex.mult(getQuick(i), tmp));
			index++;
		}
