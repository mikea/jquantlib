 ...
 * {@link hep.aida.bin}, in particular {@link hep.aida.bin.DynamicDoubleBin1D}.
 * <p>
...
public class Statistic extends Object {
	private static final cern.jet.math.DoubleFunctions F = cern.jet.math.DoubleFunctions.functions;
	 ...
	 * @see Formatter
	 * @see hep.aida.bin.DoubleBinFunction1D
	 * @see hep.aida.bin.DoubleBinFunctions1D
	 */
	public static DoubleMatrix2D aggregate(DoubleMatrix2D matrix, hep.aida.bin.DoubleBinFunction1D[] aggr, DoubleMatrix2D result) {
	 ...
	 */
	public static hep.aida.DoubleIHistogram2D cube(DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D weights) {
		...
		sorted.sort();
		cern.jet.stat.DoubleDescriptive.frequencies(sorted, distinct, null);
		...
		distinct.trimToSize();
		hep.aida.DoubleIAxis xaxis = new hep.aida.ref.DoubleVariableAxis(distinct.elements());
		...
		if (distinct.size() > 0)
			distinct.add(distinct.get(distinct.size() - 1) + epsilon);
		distinct.trimToSize();
		hep.aida.DoubleIAxis yaxis = new hep.aida.ref.DoubleVariableAxis(distinct.elements());

		hep.aida.DoubleIHistogram2D histo = new hep.aida.ref.DoubleHistogram2D("Cube", xaxis, yaxis);
	 ...
	 */
	public static hep.aida.DoubleIHistogram3D cube(DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D z, DoubleMatrix1D weights) {
		...
		if (distinct.size() > 0)
			distinct.add(distinct.get(distinct.size() - 1) + epsilon);
		distinct.trimToSize();
		hep.aida.DoubleIAxis zaxis = new hep.aida.ref.DoubleVariableAxis(distinct.elements());

		hep.aida.DoubleIHistogram3D histo = new hep.aida.ref.DoubleHistogram3D("Cube", xaxis, yaxis, zaxis);
	 ...
	 */
	public static hep.aida.DoubleIHistogram1D histogram(hep.aida.DoubleIHistogram1D histo, DoubleMatrix1D vector) {
	 ...
	 */
	public static hep.aida.DoubleIHistogram2D histogram(hep.aida.DoubleIHistogram2D histo, DoubleMatrix1D x, DoubleMatrix1D y) {
		if (x.size() != y.size())
			throw new IllegalArgumentException("vectors must have same size");
	 ...
	 *             <tt>x.size() != y.size() || y.size() != weights.size()</tt>.
	 */
	public static hep.aida.DoubleIHistogram2D histogram(hep.aida.DoubleIHistogram2D histo, DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D weights) {
		if (x.size() != y.size() || y.size() != weights.size())
			throw new IllegalArgumentException("vectors must have same size");
		for (int i = x.size(); --i >= 0;) {
	 ...
	 *             <tt>x.size() != y.size() || x.size() != z.size() || x.size() != weights.size()</tt>.
	 */
	public static hep.aida.DoubleIHistogram3D histogram(hep.aida.DoubleIHistogram3D histo, DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix1D z, DoubleMatrix1D weights) {
		if (x.size() != y.size() || x.size() != z.size() || x.size() != weights.size())
			throw new IllegalArgumentException("vectors must have same size");
		for (int i = x.size(); --i >= 0;) {
	 ...
	 * @see cern.jet.random.sampling.DoubleRandomSampler
	 */
	public static DoubleMatrix1D viewSample(DoubleMatrix1D matrix, double fraction, DoubleRandomEngine randomGenerator) {
		...
		if (randomGenerator == null)
			randomGenerator = new cern.jet.random.engine.DoubleMersenneTwister((int) System.currentTimeMillis());
		...
		int n = ncols;
		int N = matrix.size();
		cern.jet.random.sampling.DoubleRandomSampler.sample(n, N, n, 0, selected, 0, randomGenerator);
