...
public class Formatter extends AbstractFormatter {
	...
	public static void demo7() {
		// parameters
		/*
		 * double[][] values = { {3, 0, -3.4, 0}, {5.1 ,0, +3.0123456789, 0},
		...
		hep.aida.bin.DoubleBinFunction1D[] aggr = { F.mean, F.rms, F.quantile(0.25), F.median, F.quantile(0.75), F.stdDev, F.min, F.max };
		String format = "%1.2G";
		...
		// Formatter(format).toTitleString(cern.colt.matrix.DoubleFactory2D.dense.make(values),rowNames,columnNames,rowAxisName,columnAxisName,title,
		// aggr)));
	}
	 ...
	 */
	public String toTitleString(DoubleMatrix2D matrix, String[] rowNames, String[] columnNames, String rowAxisName, String columnAxisName, String title, hep.aida.bin.DoubleBinFunction1D[] aggr) {
		if (matrix.size() == 0)
			return "Empty matrix";
		...
		cern.colt.matrix.doublealgo.Statistic.aggregate(matrix, aggr, colStats); // aggregate
		// an
		// entire
		// column
		// at a
		// time
		cern.colt.matrix.doublealgo.Statistic.aggregate(matrix.viewDice(), aggr, rowStats.viewDice()); // aggregate
		// an
		// entire
		// row
		// at a
	 ...
	 * @see hep.aida.bin.DoubleBinFunctions1D
	 */
	public String toTitleString(DoubleMatrix3D matrix, String[] sliceNames, String[] rowNames, String[] columnNames, String sliceAxisName, String rowAxisName, String columnAxisName, String title, hep.aida.bin.DoubleBinFunction1D[] aggr) {
		if (matrix.size() == 0)
			return "Empty matrix";
		StringBuffer buf = new StringBuffer();
