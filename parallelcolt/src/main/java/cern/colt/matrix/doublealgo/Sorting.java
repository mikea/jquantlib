...
public class Sorting extends cern.colt.PersistentObject {
	 ...
	 * @return sorted indices
	 */
	public int[] sortIndexMulti(final DoubleMatrix1D vector) {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
	 ...
	 * @return sorted indices
	 */
	public int[] sortIndexMulti(final DoubleMatrix1D vector, IntComparator comp) {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
	 ...
	 * @return sorted indices
	 */
	public int[] sortIndex(final DoubleMatrix1D vector, final cern.colt.function.DoubleComparator c) {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
	 ...
	 */
	public int[] sortIndexMulti(final DoubleMatrix1D vector, final cern.colt.function.DoubleComparator c) throws IllegalAccessException {
		int[] indexes = new int[vector.size()]; // row indexes to reorder
		// instead of matrix itself
		for (int i = indexes.length; --i >= 0;)
			indexes[i] = i;
		IntComparator comp;
		if (!vector.isView()) {
			final double[] values = (double[]) vector.getElements();
	 ...
	 *         matrix is left unaffected.</b>
	 */
	public DoubleMatrix2D sort(DoubleMatrix2D matrix, hep.aida.bin.DoubleBinFunction1D aggregate) {
		...
		hep.aida.bin.DoubleBinFunction1D[] func = { aggregate };
	...
	public static void zdemo3() {
		...
		sorted.assign(cern.jet.math.DoubleFunctions.sin);
		/*
	...
	protected static void zdemo4() {
		...
		matrix1.assign(matrix2, cern.jet.math.DoubleFunctions.pow);
	...
	public static void zdemo5(int rows, int columns, boolean print) {
		...
		final cern.jet.math.DoubleFunctions F = cern.jet.math.DoubleFunctions.functions;
		...
		timer.reset().start();
		// THE QUICK VERSION (takes some 10 secs)
		A = sort.sort(A, hep.aida.bin.DoubleBinFunctions1D.median);
		...
		if (print) {
			...
			hep.aida.bin.DoubleBinFunction1D[] funs = { hep.aida.bin.DoubleBinFunctions1D.median, hep.aida.bin.DoubleBinFunctions1D.sumLog, hep.aida.bin.DoubleBinFunctions1D.geometricMean };
