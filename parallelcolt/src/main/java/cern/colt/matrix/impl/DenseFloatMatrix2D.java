...
public class DenseFloatMatrix2D extends FloatMatrix2D {
	 ...
	 */
	protected DenseFloatMatrix2D(int rows, int columns, float[] elements, int rowZero, int columnZero, int rowStride, int columnStride) {
	...
	public FloatMatrix2D assign(final cern.colt.function.FloatFunction function) {
		...
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			if (function instanceof cern.jet.math.FloatMult) { // x[i] =
				// mult*x[i]
	...
	public FloatMatrix2D assign(final cern.colt.function.FloatProcedure cond, final cern.colt.function.FloatFunction function) {
		final int zero = index(0, 0);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
	...
	public FloatMatrix2D assign(final FloatMatrix2D y, final cern.colt.function.FloatFloatFunction function) {
		...
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			if (function instanceof cern.jet.math.FloatPlusMult) {
	...
	public FComplexMatrix2D getFft2() {
		...
		final float[] cElems = (float[]) ((DenseFComplexMatrix2D) C).getElements();
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	...
	public FloatMatrix1D zMult(final FloatMatrix1D y, FloatMatrix1D z, final float alpha, final float beta, final boolean transposeA) {
		...
		final float[] elemsZ = (float[]) z_loc.getElements();
