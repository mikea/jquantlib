...
public class Test_DenseDComplexMatrix2D {
	...
	public void setUp() throws Exception {
		...
		a_1dt = (double[]) A.viewDice().copy().getElements();
	...
	public void testAssignImaginary() {
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				Utils.assertArrayEquals(new double[] { 0, Im.getQuick(r, c) }, A.getQuick(r, c), tol);
			}
		}
		// multiple threads
		Utils.setNP(nThreads);
		...
		for (int r = 0; r < cols; r++) {
			for (int c = 0; c < rows; c++) {
				Utils.assertArrayEquals(new double[] { 0, Imv.getQuick(r, c) }, Av.getQuick(r, c), tol);
			}
		}
		// multiple threads
		Utils.setNP(nThreads);
	...
	public void testAssignReal() {
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				Utils.assertArrayEquals(new double[] { Re.getQuick(r, c), 0 }, A.getQuick(r, c), tol);
			}
		}
		// multiple threads
		Utils.setNP(nThreads);
		...
		for (int r = 0; r < cols; r++) {
			for (int c = 0; c < rows; c++) {
				Utils.assertArrayEquals(new double[] { Rev.getQuick(r, c), 0 }, Av.getQuick(r, c), tol);
			}
		}
		// multiple threads
		Utils.setNP(nThreads);
	...
	public void testForEachNonZero() {
		...
		System.arraycopy((double[]) A.getElements(), 0, aElts, 0, rows * 2 * cols);
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				Utils.assertArrayEquals(DComplex.sqrt(new double[] { aElts[r * 2 * cols + 2 * c], aElts[r * 2 * cols + 2 * c + 1] }), A.getQuick(r, c), tol);
			}
		}
		// multiple threads
		Utils.setNP(nThreads);
		...
		for (int r = 0; r < cols; r++) {
			for (int c = 0; c < rows; c++) {
				tmp = Av.getQuick(r, c);
				Utils.assertArrayEquals(DComplex.sqrt(new double[] { aElts[r * 2 * rows + 2 * c], aElts[r * 2 * rows + 2 * c + 1] }), tmp, tol);
			}
		}
		// multiple threads
		Utils.setNP(nThreads);
	...
	public void testGetNonZeros() {
		...
		// single thread
		Utils.setNP(1);
		A = new DenseDComplexMatrix2D(cols, rows);
		DComplexMatrix2D Av = A.viewDice();
