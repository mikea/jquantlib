...
public class DenseDoubleMatrix1D extends DoubleMatrix1D {
	...
	public DoubleMatrix1D assign(final cern.colt.function.DoubleFunction function) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
	...
	public DoubleMatrix1D assign(final cern.colt.function.DoubleProcedure cond, final cern.colt.function.DoubleFunction function) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
	...
	public DComplexMatrix1D getFft() {
		...
		final double[] cElems = (double[]) ((DenseDComplexMatrix1D) c).getElements();
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
