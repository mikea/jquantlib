...
/**
 * Sparse row-compressed 2-d matrix holding <tt>double</tt> elements. First
 ...
 * {@link cern.colt.list.DoubleArrayList} to hold the column indexes and nonzero
 ...
 * </td>
 * </table> Method
 * {@link #assign(DoubleMatrix2D,cern.colt.function.DoubleDoubleFunction)} does
 * just that if you supply {@link cern.jet.math.DoubleFunctions#plusMult} as
 * argument.
 * 
 * 
...
public class RCDoubleMatrix2D extends WrapperDoubleMatrix2D {
	...
	public DoubleMatrix2D assign(final cern.colt.function.DoubleFunction function) {
		if (function instanceof cern.jet.math.DoubleMult) { // x[i] = mult*x[i]
			final double alpha = ((cern.jet.math.DoubleMult) function).multiplicator;
			if (alpha == 1)
				return this;
			if (alpha == 0)
				return assign(0);
	...
	public DoubleMatrix2D assign(DoubleMatrix2D y, cern.colt.function.DoubleDoubleFunction function) {
		...
		if (function instanceof cern.jet.math.DoublePlusMult) { // x[i] = x[i] +
			// alpha*y[i]
			final double alpha = ((cern.jet.math.DoublePlusMult) function).multiplicator;
			if (alpha == 0)
				return this; // nothing to do
		...
		if (function == cern.jet.math.DoubleFunctions.div) { // x[i] = x[i] /
																// y[i]
			int[] idx = indexes.elements();
			double[] vals = values.elements();
	...
	public DoubleMatrix1D zMult(DoubleMatrix1D y, DoubleMatrix1D z, double alpha, double beta, boolean transposeA) {
		...
		if (!transposeA) {
			...
			if (!ignore)
				z.assign(cern.jet.math.DoubleFunctions.mult(beta));
			for (int i = 0; i < s; i++) {
				int high = starts[i + 1];
	...
	public DoubleMatrix2D zMult(DoubleMatrix2D B, DoubleMatrix2D C, final double alpha, double beta, boolean transposeA, boolean transposeB) {
		...
		if (!ignore)
			C.assign(cern.jet.math.DoubleFunctions.mult(beta));
		...
		final cern.jet.math.DoublePlusMult fun = cern.jet.math.DoublePlusMult.plusMult(0);
