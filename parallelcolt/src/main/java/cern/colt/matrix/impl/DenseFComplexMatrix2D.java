 ...
 * <p>
 * <tt>memory [bytes] = 8*rows()*2*columns()</tt>. Thus, a 1000*1000 matrix
 * uses 16 MB.
 * <p>
 * <b>Time complexity:</b>
 * <p>
...
public class DenseFComplexMatrix2D extends FComplexMatrix2D {
	 ...
	 */
	protected DenseFComplexMatrix2D(int rows, int columns, float[] elements, int rowZero, int columnZero, int rowStride, int columnStride) {
	...
	public FComplexMatrix2D assign(final cern.colt.function.FComplexFComplexFunction function) {
		final int zero = index(0, 0);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
	...
	public FComplexMatrix2D assign(final cern.colt.function.FComplexProcedure cond, final cern.colt.function.FComplexFComplexFunction function) {
		final int zero = index(0, 0);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
	...
	public FComplexMatrix2D assign(final cern.colt.function.FComplexRealFunction function) {
		final int zero = index(0, 0);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
	...
	public FComplexMatrix2D assign(final FComplexMatrix2D y, final cern.colt.function.FComplexFComplexFComplexFunction function) {
		// overriden for performance only
	...
	public FComplexMatrix1D vectorize() {
		...
		final float[] elemsOther = (float[]) v.getElements();
		int np = Utils.getNP();
		if ((np > 1) && (rows * columns >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = columns / np;
	...
	public FComplexMatrix1D zMult(final FComplexMatrix1D y, FComplexMatrix1D z, final float[] alpha, final float[] beta, boolean transposeA) {
		...
		final float[] elemsY = (float[]) y.getElements();
		final float[] elemsZ = (float[]) zLoc.getElements();
