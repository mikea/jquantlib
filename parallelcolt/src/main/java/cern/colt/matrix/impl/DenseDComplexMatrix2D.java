...
public class DenseDComplexMatrix2D extends DComplexMatrix2D {
	 ...
	 */
	protected DenseDComplexMatrix2D(int rows, int columns, double[] elements, int rowZero, int columnZero, int rowStride, int columnStride) {
	...
	public DComplexMatrix2D assign(final cern.colt.function.DComplexDComplexFunction function) {
		final int zero = index(0, 0);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
	...
	public DComplexMatrix2D assign(final cern.colt.function.DComplexProcedure cond, final cern.colt.function.DComplexDComplexFunction function) {
		final int zero = index(0, 0);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
	...
	public DComplexMatrix2D assign(final cern.colt.function.DComplexRealFunction function) {
		final int zero = index(0, 0);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
	...
	public DComplexMatrix2D assign(final DComplexMatrix2D y, final cern.colt.function.DComplexDComplexDComplexFunction function) {
		// overriden for performance only
	...
	public DComplexMatrix1D zMult(final DComplexMatrix1D y, DComplexMatrix1D z, final double[] alpha, final double[] beta, boolean transposeA) {
		...
		final double[] elemsZ = (double[]) zLoc.getElements();
