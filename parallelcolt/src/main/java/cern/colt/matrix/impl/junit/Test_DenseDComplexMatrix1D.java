...
public class Test_DenseDComplexMatrix1D {
	...
	public void testAssignComplexComplexFunction() {
		...
		for (int i = 0; i < a.length; i = i + 2) {
			...
			tmp[1] = a[i + 1];
			tmp = DComplex.acos(tmp);
			Utils.assertArrayEquals(tmp, Av.getQuick(size - 1 - (i / 2)), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testAssignComplexMatrix1DComplexComplexFunction() {
		...
		for (int i = 0; i < aElts.length; i = i + 2) {
			...
			tmp2[1] = b[i + 1];
			tmp1 = DComplex.div(tmp1, tmp2);
			Utils.assertArrayEquals(tmp1, Av.getQuick(size - 1 - (i / 2)), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testAssignComplexProcedureComplexComplexFunction() {
		...
		for (int i = 0; i < B.size(); i++) {
			Utils.assertArrayEquals(Bv.getQuick(i), Av.getQuick(i), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testAssignDoubleArray() {
		...
		for (int i = 0; i < a.length; i = i + 2) {
			tmp[0] = a[i];
			tmp[1] = a[i + 1];
			Utils.assertArrayEquals(tmp, Av.getQuick(i / 2), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testAssignDoubleDouble() {
		...
		for (int i = 0; i < a.length; i = i + 2) {
			tmp = Av.getQuick(i / 2);
			Utils.assertArrayEquals(value, tmp, tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testCopy() {
		...
		B = Av.copy();
		for (int i = 0; i < Av.size(); i++) {
			Utils.assertArrayEquals(Av.get(i), B.get(i), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testGetNonZerosIntArrayListArrayListOfdouble() {
		...
		Utils.setNP(1);
		DComplexMatrix1D A = new DenseDComplexMatrix1D(size);
		A.setQuick(size / 4, 1, 1);
		A.setQuick(size / 2, 0, 2);
		A.setQuick(size - 1, 3, 0);
		...
		A = new DenseDComplexMatrix1D(size);
		DComplexMatrix1D Av = A.viewFlip();
		Av.setQuick(size / 4, 1, 1);
		Av.setQuick(size / 2, 0, 2);
		Av.setQuick(size - 1, 3, 0);
		indexList = new IntArrayList();
		valueList = new ArrayList<double[]>();
	...
	public void testReshapeIntInt() {
		...
		for (int c = 0; c < cols; c++) {
			for (int r = 0; r < rows; r++) {
				Utils.assertArrayEquals(A.getQuick(idx++), B.getQuick(r, c), tol);
			}
		}
		// multiple threads
		Utils.setNP(nThreads);
		...
		for (int c = 0; c < cols; c++) {
			for (int r = 0; r < rows; r++) {
				Utils.assertArrayEquals(Av.getQuick(idx++), B.getQuick(r, c), tol);
			}
		}
		// multiple threads
		Utils.setNP(nThreads);
	...
	public void testReshapeIntIntInt() {
		...
		for (int s = 0; s < slices; s++) {
			for (int c = 0; c < cols; c++) {
				for (int r = 0; r < rows; r++) {
					Utils.assertArrayEquals(A.getQuick(idx++), B.getQuick(s, r, c), tol);
				}
			}
		}
		// multiple threads
		...
		for (int s = 0; s < slices; s++) {
			for (int c = 0; c < cols; c++) {
				for (int r = 0; r < rows; r++) {
					Utils.assertArrayEquals(Av.getQuick(idx++), B.getQuick(s, r, c), tol);
				}
			}
		}
		// multiple threads
	...
	public void testSwap() {
		...
		for (int i = 0; i < aElts.length; i = i + 2) {
			...
			tmp[0] = a[i];
			tmp[1] = a[i + 1];
			Utils.assertArrayEquals(tmp, Bv.getQuick(size - 1 - i / 2), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testViewFlip() {
		...
		for (int i = 0; i < size; i++) {
			Utils.assertArrayEquals(A.getQuick(size - 1 - i), B.getQuick(i), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testViewPart() {
		...
		for (int i = 0; i < 11; i++) {
			Utils.assertArrayEquals(new double[] { a[30 + 2 * i], a[30 + 1 + 2 * i] }, B.getQuick(i), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testViewSelectionIntArray() {
		...
		for (int i = 0; i < indexes.length; i++) {
			Utils.assertArrayEquals(A.getQuick(indexes[i]), B.getQuick(i), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testViewStrides() {
		...
		for (int i = 0; i < B.size(); i++) {
			Utils.assertArrayEquals(A.getQuick(i * stride), B.getQuick(i), tol);
		}
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
	...
	public void testZDotProductComplexMatrix1D() {
		...
		Utils.assertArrayEquals(tmpSumMult, product, tol);
		// multiple threads
		Utils.setNP(nThreads);
		Utils.setThreadsBeginN_1D(nThreadsBegin);
		A = new DenseDComplexMatrix1D(a);
