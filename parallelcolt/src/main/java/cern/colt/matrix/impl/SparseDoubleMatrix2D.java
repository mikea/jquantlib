 ...
 * {@link cern.colt.map.OpenIntDoubleHashMap}, which is a compact and
...
public class SparseDoubleMatrix2D extends DoubleMatrix2D {
	 ...
	 * related to memory usage see {@link cern.colt.map.OpenIntDoubleHashMap}.
	 * 
	 * @param rows
	 *            the number of rows the matrix shall have.
	 * @param columns
	...
	public DoubleMatrix2D assign(cern.colt.function.DoubleFunction function) {
		if (this.isNoView && function instanceof cern.jet.math.DoubleMult) { // x[i]
			// =
			// mult*x[i]
	...
	public DoubleMatrix1D zMult(DoubleMatrix1D y, DoubleMatrix1D z, double alpha, double beta, final boolean transposeA) {
		...
		if (!ignore)
			z.assign(cern.jet.math.DoubleFunctions.mult(beta / alpha));
		...
		if (alpha != 1)
			z.assign(cern.jet.math.DoubleFunctions.mult(alpha));
		return z;
	}
