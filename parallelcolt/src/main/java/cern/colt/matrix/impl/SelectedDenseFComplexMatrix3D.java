...
class SelectedDenseFComplexMatrix3D extends FComplexMatrix3D {
	 ...
	 */
	protected SelectedDenseFComplexMatrix3D(float[] elements, int[] sliceOffsets, int[] rowOffsets, int[] columnOffsets, int offset) {
	...
	public float[] getQuick(int slice, int row, int column) {
		...
		return new float[] { elements[offset + sliceOffsets[idxs] + rowOffsets[idxr] + columnOffsets[idxc]], elements[offset + sliceOffsets[idxs] + rowOffsets[idxr] + columnOffsets[idxc] + 1] };
	}
	...
	public FComplexMatrix2D viewColumn(int column) {
		...
		return new SelectedDenseFComplexMatrix2D(viewRows, viewColumns, this.elements, viewRowZero, viewColumnZero, viewRowStride, viewColumnStride, viewRowOffsets, viewColumnOffsets, viewOffset);
	}
