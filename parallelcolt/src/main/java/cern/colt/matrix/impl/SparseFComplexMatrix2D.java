...
public class SparseFComplexMatrix2D extends FComplexMatrix2D {
	 ...
	 */
	public FComplexMatrix2D assign(final FComplexMatrix2D y, cern.colt.function.FComplexFComplexFComplexFunction function) {
		if (!this.isNoView)
			return super.assign(y, function);

		checkShape(y);

		if (function instanceof cern.jet.math.FComplexPlusMult) {
			// x[i] = x[i] + alpha*y[i]
			final float[] alpha = ((cern.jet.math.FComplexPlusMult) function).multiplicator;
			if (alpha[0] == 0 && alpha[1] == 1)
				return this; // nothing to do
			y.forEachNonZero(new cern.colt.function.IntIntFComplexFunction() {
	 ...
	 * @return the elements
	 */
	public ConcurrentHashMap<Integer, float[]> getElements() {
		return elements;
	}
