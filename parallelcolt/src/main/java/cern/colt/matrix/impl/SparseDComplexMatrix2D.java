...
public class SparseDComplexMatrix2D extends DComplexMatrix2D {
	 ...
	 */
	public DComplexMatrix2D assign(final DComplexMatrix2D y, cern.colt.function.DComplexDComplexDComplexFunction function) {
		if (!this.isNoView)
			return super.assign(y, function);

		checkShape(y);

		if (function instanceof cern.jet.math.DComplexPlusMult) {
			// x[i] = x[i] + alpha*y[i]
			final double[] alpha = ((cern.jet.math.DComplexPlusMult) function).multiplicator;
			if (alpha[0] == 0 && alpha[1] == 1)
				return this; // nothing to do
			y.forEachNonZero(new cern.colt.function.IntIntDComplexFunction() {
