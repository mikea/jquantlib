...
/**
 * Dense 1-d matrix (aka <i>vector</i>) holding <tt>float</tt> elements.
...
public class DenseFloatMatrix1D extends FloatMatrix1D {
	...
	public FloatMatrix1D assign(final cern.colt.function.FloatFunction function) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			if (function instanceof cern.jet.math.FloatMult) {
				// x[i] = mult*x[i]
				float multiplicator = ((cern.jet.math.FloatMult) function).multiplicator;
				if (multiplicator == 1)
					return this;
			}
			Future[] futures = new Future[np];
	...
	public FloatMatrix1D assign(final cern.colt.function.FloatProcedure cond, final cern.colt.function.FloatFunction function) {
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = Utils.threadPool.submit(new Runnable() {
					public void run() {
						...
						if (function == cern.jet.math.FloatFunctions.mult) {
							// x[i] = x[i] * y[i]
							idx = zero + startidx * stride;
						...
						} else if (function == cern.jet.math.FloatFunctions.div) {
							// x[i] = x[i] / y[i]
							idx = zero + startidx * stride;
							idxOther = zeroOther + startidx * strideOther;
							for (int k = startidx; k < stopidx; k++) {
	...
	public FComplexMatrix1D getFft() {
		...
		final float[] cElems = (float[]) ((DenseFComplexMatrix1D) c).getElements();
		int np = Utils.getNP();
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			Future[] futures = new Future[np];
			int k = size / np;
			for (int j = 0; j < np; j++) {
				...
				futures[j] = Utils.threadPool.submit(new Runnable() {
					public void run() {
						System.arraycopy(elements, startidx, cElems, startidx, length);
					}
				});
			}
			try {
			...
			System.arraycopy(elements, 0, cElems, 0, size);
		}
		if (fft == null) {
	...
	public float zDotProduct(FloatMatrix1D y, int from, int length) {
		...
		float sum = 0;
		int np = Utils.getNP();
		if ((np > 1) && (length >= Utils.getThreadsBeginN_1D())) {
