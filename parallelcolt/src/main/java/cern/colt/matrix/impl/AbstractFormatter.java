...
public abstract class AbstractFormatter extends cern.colt.PersistentObject {
	...
	public static void demo1() {
		 ...
		 * "%1.10G", "%f", "%1.2f", "%0.2e", null};
		 *  // now the processing int size = formats.length; ObjectMatrix2D
		 * matrix = cern.colt.matrix.ObjectFactory2D.dense.make(values);
		 * String[] strings = new String[size]; String[] sourceCodes = new
		 * String[size]; String[] htmlStrings = new String[size]; String[]
		 * htmlSourceCodes = new String[size];
		 * 
	...
	public static void demo2() {
		 ...
		 * "%f", "%1.2f", "%0.2e"}; String[] formats = {"%G", "%1.19G"};
		 *  // now the processing int size = formats.length; ObjectMatrix1D
		 * matrix = new DenseObjectMatrix1D(values);
		 * 
