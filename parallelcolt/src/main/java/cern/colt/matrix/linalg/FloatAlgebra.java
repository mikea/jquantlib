...
import cern.colt.matrix.impl.DenseFComplexMatrix1D;
...
/**
 * Linear algebraic matrix operations operating on {@link FloatMatrix2D};
 ...
 * @version 1.1 10/20/2007
 */
public class FloatAlgebra extends cern.colt.PersistentObject {
	...
	public FloatCholeskyDecomposition chol(FloatMatrix2D matrix) {
		return new FloatCholeskyDecomposition(matrix);
	}
	 ...
	 */
	public Object clone() {
		return new FloatAlgebra(property.tolerance());
	}
	...
	public FloatEigenvalueDecomposition eig(FloatMatrix2D matrix) {
		return new FloatEigenvalueDecomposition(matrix);
	}
	 ...
	 * Returns sqrt(a^2 + b^2) without under/overflow.
	 */
	protected static cern.colt.function.FloatFloatFunction hypotFunction() {
		return new cern.colt.function.FloatFloatFunction() {
	...
	public FloatMatrix2D inverse(FloatMatrix2D A) {
		if (property.isSquare(A) && property.isDiagonal(A)) {
			...
			boolean isNonSingular = FloatDiagonal.inverse(inv);
			if (!isNonSingular)
	...
	public FloatLUDecomposition lu(FloatMatrix2D matrix) {
		return new FloatLUDecomposition(matrix);
	}

	public FComplexMatrix1D kron(FComplexMatrix1D x, FComplexMatrix1D y) {
		int size_x = x.size();
		int size_y = y.size();
		FComplexMatrix1D C = new DenseFComplexMatrix1D(size_x * size_y);
		for (int i = 0; i < size_x; i++) {
			C.viewPart(i * size_y, size_y).assign(y);
			C.viewPart(i * size_y, size_y).assign(FComplexFunctions.mult(x.getQuick(i)));
		}
		return C;
	}

	public FloatMatrix1D kron(float[] A, float[] B) {
		int size_a = A.length;
		int size_b = B.length;
		FloatMatrix1D C = new DenseFloatMatrix1D(size_a * size_b);
		for (int i = 0; i < size_a; i++) {
			C.viewPart(i * size_b, size_b).assign(B);
			C.viewPart(i * size_b, size_b).assign(FloatFunctions.mult(A[i]));
		}
		return C;
	}
	...
	public FloatMatrix1D kron(FloatMatrix1D x, FloatMatrix1D y) {
		int size_x = x.size();
		int size_y = y.size();
		FloatMatrix1D C = new DenseFloatMatrix1D(size_x * size_y);
		for (int i = 0; i < size_x; i++) {
			C.viewPart(i * size_y, size_y).assign(y);
			C.viewPart(i * size_y, size_y).assign(FloatFunctions.mult(x.getQuick(i)));
		}
		return C;
	}
	...
	public FloatMatrix2D multOuter(FloatMatrix1D x, FloatMatrix1D y, FloatMatrix2D A) {
		...
		for (int column = columns; --column >= 0;)
			A.viewColumn(column).assign(x, cern.jet.math.FloatFunctions.mult);
		return A;
	}
	...
	public float norm1(FloatMatrix1D x) {
		if (x.size() == 0)
			return 0;
		return x.aggregate(cern.jet.math.FloatFunctions.plus, cern.jet.math.FloatFunctions.abs);
	}
	...
	public float vectorNorm2(final FloatMatrix2D X) {
		if (X.isView() == true || !(X instanceof DenseFloatMatrix2D)) {
			...
			float sum = 0;
			int np = Utils.getNP();
			if ((np > 1) && (rows * cols >= cern.colt.Utils.getThreadsBeginN_2D())) {
			...
			final float[] elems = (float[]) ((DenseFloatMatrix2D) X).getElements();
			float sum = 0;
			int np = Utils.getNP();
			if ((np > 1) && (elems.length >= cern.colt.Utils.getThreadsBeginN_2D())) {
				Future[] futures = new Future[np];
				Float result;
	...
	public float vectorNorm2(final FloatMatrix3D X) {
		if (X.isView() == true || !(X instanceof DenseFloatMatrix3D)) {
			...
			final float[] elems = (float[]) ((DenseFloatMatrix3D) X).getElements();
			float sum = 0;
			int np = Utils.getNP();
			if ((np > 1) && (elems.length >= cern.colt.Utils.getThreadsBeginN_2D())) {
				Future[] futures = new Future[np];
	...
	public float normF(FloatMatrix2D A) {
		if (A.size() == 0)
			return 0;
		return A.aggregate(hypotFunction(), cern.jet.math.FloatFunctions.identity);
	}
	...
	public float normF(FComplexMatrix2D A) {
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				tmp = FComplex.abs(A.getQuick(r, c));
				norm += tmp * tmp;
			}
		}
	...
	public float normInfinity(FloatMatrix1D x) {
		...
		if (x.size() == 0)
			return 0;
		return x.aggregate(cern.jet.math.FloatFunctions.max, cern.jet.math.FloatFunctions.abs);
		// if (x.size()==0) return 0;
		// return
		// x.aggregate(cern.jet.math.Functions.plus,cern.jet.math.Functions.abs);
		// float max = 0;
	...
	public FloatMatrix1D permute(FloatMatrix1D A, int[] indexes, float[] work) {
		...
		/*
		 * int i=size; int a; while (--i >= 0 && (a=indexes[i])==i) if (a < 0 ||
		 * a >= size) throw new IndexOutOfBoundsException("invalid
		 * permutation"); if (i<0) return; // nothing to permute
		 */
	...
	public FloatQRDecomposition qr(FloatMatrix2D matrix) {
		return new FloatQRDecomposition(matrix);
	}
	...
	public FloatMatrix1D forwardSolve(final FloatMatrix2D L, final FloatMatrix1D b) {
		...
		for (int r = 1; r < rows; r++) {
			sum = 0;
			for (int c = 0; c < r; c++) {
				sum += L.getQuick(r, c) * b.getQuick(c);
			}
	...
	public FloatSingularValueDecomposition svd(FloatMatrix2D matrix) {
		return new FloatSingularValueDecomposition(matrix);
	}
	...
	public FloatSingularValueDecompositionDC svdDC(FloatMatrix2D matrix) {
		return new FloatSingularValueDecompositionDC(matrix, true, true);
	}
	...
	public String toVerboseString(FloatMatrix2D matrix) {
		/*
		 * StringBuffer buf = new StringBuffer(); String unknown = "Illegal
		 * operation or error: "; String constructionException = "Illegal
		 * operation or error upon construction: ";
		 * 
		 * buf.append("------------------------------------------------------------------\n");
		 * buf.append("LUDecomposition(A) --> isNonSingular, det, pivot, L, U,
		 * inverse(A)\n");
		 * buf.append("------------------------------------------------------------------\n");
		 */
		...
		FloatLUDecomposition lu = null;
		try {
			lu = new FloatLUDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
		...
		FloatQRDecomposition qr = null;
		try {
			qr = new FloatQRDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
		...
		FloatCholeskyDecomposition chol = null;
		try {
			chol = new FloatCholeskyDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
		...
		FloatEigenvalueDecomposition eig = null;
		try {
			eig = new FloatEigenvalueDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
		...
		FloatSingularValueDecomposition svd = null;
		try {
			svd = new FloatSingularValueDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
	 ...
	 * {@link FloatMatrix2D#viewDice A.viewDice()}. This is a zero-copy
	...
	public FloatMatrix2D xmultOuter(FloatMatrix1D x, FloatMatrix1D y) {
		FloatMatrix2D A = x.like2D(x.size(), y.size());
		multOuter(x, y, A);
		return A;
	}
