 ...
 * <p>
 * Except where explicitly indicated, all methods involving equality tests (<tt>==</tt>)
 * allow for numerical instability, to a degree specified upon instance
 * construction and returned by method {@link #tolerance()}. The public static
 * final variable <tt>DEFAULT</tt> represents a default Property object with a
 * tolerance of <tt>1.0E-9</tt>. The public static final variable
 * <tt>ZERO</tt> represents a Property object with a tolerance of <tt>0.0</tt>.
 * The public static final variable <tt>TWELVE</tt> represents a Property
 * object with a tolerance of <tt>1.0E-12</tt>. As long as you are happy with
 * these tolerances, there is no need to construct Property objects. Simply use
 * idioms like <tt>Property.DEFAULT.equals(A,B)</tt>,
 * <tt>Property.ZERO.equals(A,B)</tt>, <tt>Property.TWELVE.equals(A,B)</tt>.
 * <p>
 * To work with a different tolerance (e.g. <tt>1.0E-15</tt> or
 * <tt>1.0E-5</tt>) use the constructor and/or method
 * {@link #setTolerance(float)}. Note that the public static final Property
 * objects are immutable: Is is not possible to alter their tolerance. Any
 ...
 * 
 * </td>
 * </table> Here are some example properties <table border="1" cellspacing="0">
 ...
 * <td align="center" valign="middle"> <div align="center"><tt>1</tt></div>
 * </td>
 * <td align="center" valign="middle"> <div align="center"><tt>2</tt></div>
 * </td>
 * </tr>
 * <tr align="center" valign="middle">
 ...
 * <td align="center" valign="middle"> <div align="center"><tt>3</tt></div>
 * </td>
 * <td align="center" valign="middle"> <div align="center"><tt>2</tt></div>
 * </td>
 * </tr>
 ...
 * <td align="center" valign="middle"> <div align="center"><tt>4</tt></div>
 * </td>
 * <td align="center" valign="middle"> <div align="center"><tt>3</tt></div>
 * </td>
 * </tr>
 ...
 * <td align="center" valign="middle"> <div align="center"><tt>unstructured</tt></div>
 * </td>
 * <td align="center" valign="middle"> <div align="center"><tt>unstructured</tt></div>
 * </td>
 * </tr>
...
public class FloatProperty extends cern.colt.PersistentObject {
	...
	public void checkRectangular(FloatMatrix2D A) {
		if (A.rows() < A.columns()) {
			throw new IllegalArgumentException("Matrix must be rectangular: " + cern.colt.matrix.floatalgo.Formatter.shape(A));
		}
	}
	...
	public void checkSquare(FloatMatrix2D A) {
		if (A.rows() != A.columns())
			throw new IllegalArgumentException("Matrix must be square: " + cern.colt.matrix.floatalgo.Formatter.shape(A));
	}
	...
	/**
	 * Returns whether all cells of the given matrix <tt>A</tt> are equal to
	 * the given value. The result is <tt>true</tt> if and only if
	 * <tt>A != null</tt> and
	...
	public boolean equals(final FloatMatrix1D A, final float value) {
		...
		if ((np > 1) && (size >= Utils.getThreadsBeginN_1D())) {
			...
			for (int j = 0; j < np; j++) {
				...
				futures[j] = Utils.threadPool.submit(new Callable<Boolean>() {
	...
	/**
	 * Returns whether both given matrices <tt>A</tt> and <tt>B</tt> are
	 * equal. The result is <tt>true</tt> if <tt>A==B</tt>. Otherwise, the
	 * result is <tt>true</tt> if and only if both arguments are
	 * <tt>!= null</tt>, have the same size and
	...
	public boolean equals(final FloatMatrix2D A, final float value) {
		...
		final float epsilon = tolerance();
		int np = Utils.getNP();
		if ((np > 1) && (A.size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			Boolean[] results = new Boolean[np];
			int k = A.rows() / np;
			for (int j = 0; j < np; j++) {
	 ...
	 * equal. The result is <tt>true</tt> if <tt>A==B</tt>. Otherwise, the
	 * result is <tt>true</tt> if and only if both arguments are
	 * <tt>!= null</tt>, have the same number of columns and rows and
	...
	public boolean equals(final FloatMatrix3D A, final float value) {
		...
		final float epsilon = tolerance();
		int np = Utils.getNP();
		if ((np > 1) && (A.size() >= Utils.getThreadsBeginN_3D())) {
			Future[] futures = new Future[np];
			Boolean[] results = new Boolean[np];
			int k = slices / np;
			for (int j = 0; j < np; j++) {
	 ...
	 * equal. The result is <tt>true</tt> if <tt>A==B</tt>. Otherwise, the
	 * result is <tt>true</tt> if and only if both arguments are
	 * <tt>!= null</tt>, have the same number of columns, rows and slices,
	 * and
	...
	public void generateNonSingular(FloatMatrix2D A) {
		checkSquare(A);
		cern.jet.math.FloatFunctions F = cern.jet.math.FloatFunctions.functions;
	...
	/**
	 * A matrix <tt>A</tt> is <i>diagonal</i> if <tt>A[i,j] == 0</tt>
	...
	/**
	 * A matrix <tt>A</tt> is <i>diagonally dominant by row</i> if the
	 * absolute value of each diagonal element is larger than the sum of the
	 * absolute values of the off-diagonal elements in the corresponding row.
	...
	/**
	 * A matrix <tt>A</tt> is an <i>identity</i> matrix if
	 * <tt>A[i,i] == 1</tt> and all other cells are zero. Matrix may but need
	 * not be square.
	 */
	...
	public boolean isOrthogonal(FloatMatrix2D A) {
		checkSquare(A);
		return equals(A.zMult(A, null, 1, 0, false, true), cern.colt.matrix.FloatFactory2D.dense.identity(A.rows()));
	}

	/**
	 * A matrix <tt>A</tt> is <i>positive</i> if <tt>A[i,j] &gt; 0</tt>
	 * holds for all cells.
	 * <p>
	 * Note: Ignores tolerance.
	 */
	 ...
	 * <tt>A[i,j]==0</tt> whenever <tt>i &lt;= j</tt>. Matrix may but need
	 * not be square.
	 */
	...
	/**
	 * A matrix <tt>A</tt> is <i>strictly triangular</i> if it is triangular
	 * and its diagonal elements all equal 0. Matrix may but need not be square.
	 */
	...
	/**
	 * A matrix <tt>A</tt> is <i>symmetric</i> if <tt>A = tranpose(A)</tt>,
	 * that is <tt>A[i,j] == A[j,i]</tt>.
	 * 
	 * @throws IllegalArgumentException
	 *             if <tt>!isSquare(A)</tt>.
	 */
	...
	/**
	 * A matrix <tt>A</tt> is <i>triangular</i> iff it is either upper or
	 * lower triangular. Matrix may but need not be square.
	 */
	...
	/**
	 * A matrix <tt>A</tt> is <i>tridiagonal</i> if <tt>A[i,j]==0</tt>
	 * whenever <tt>Math.abs(i-j) > 1</tt>. Matrix may but need not be
	 * square.
	 */
	...
	/**
	 * A matrix <tt>A</tt> is <i>unit triangular</i> if it is triangular and
	 * its diagonal elements all equal 1. Matrix may but need not be square.
	 */
	...
	/**
	 * The <i>lower bandwidth</i> of a square matrix <tt>A</tt> is the
	 * maximum <tt>i-j</tt> for which <tt>A[i,j]</tt> is nonzero and
	 * <tt>i &gt; j</tt>. A <i>banded</i> matrix has a "band" about the
	 * diagonal. Diagonal, tridiagonal and triangular matrices are special
	 * cases.
	 * 
	 * @param A
	 ...
	 * bandwidth</i> is the maximum <tt>i-j</tt> for which <tt>A[i,j]</tt>
	 * is nonzero and <tt>i &gt; j</tt>. Diagonal, tridiagonal and triangular
	 * matrices are special cases.
	 * <p>
	 * Examples: <table border="1" cellspacing="0">
	 * <tr align="left" valign="top">
	 * <td valign="middle" align="left"><tt>matrix</tt></td>
	 * <td> <tt>4&nbsp;x&nbsp;4&nbsp;<br>
	 0&nbsp;0&nbsp;0&nbsp;0<br>
	...
	/**
	 * Returns summary information about the given matrix <tt>A</tt>. That is
	 * a String with (propertyName, propertyValue) pairs. Useful for debugging
	 * or to quickly get the rough picture of a matrix. For example,
	 * 
	 * <pre>
	 * 	 density                      : 0.9
	 * 	 isDiagonal                   : false
	 ...
	 * maximum <tt>j-i</tt> for which <tt>A[i,j]</tt> is nonzero and
	 * <tt>j &gt; i</tt>. A <i>banded</i> matrix has a "band" about the
	 * diagonal. Diagonal, tridiagonal and triangular matrices are special
	 * cases.
	 * 
	 * @param A
