...
public class DoubleSingularValueDecompositionDC {
	 ...
	 */
	public DoubleSingularValueDecompositionDC(DoubleMatrix2D Arg, boolean wantUV, boolean wantWholeUV) {
		this.wantUV = wantUV;
		this.wantWholeUV = wantWholeUV;
		m = Arg.rows();
		n = Arg.columns();
		double[] elems = (double[]) Arg.viewDice().copy().getElements();
		minmn = Math.min(m, n);
		...
		if (wantUV = true) {
			if (wantWholeUV) { // JOBZ='A'
				...
				Dgesdd.dgesdd("A", m, n, elems, 0, m, s, 0, Ut, 0, m, V, 0, n, work, 0, lwork, iwork, 0, info);
			} else { // JOBZ='S'
				Ut = new double[m * minmn];
				V = new double[minmn * n];
				lwork = 3 * minmn * minmn + Math.max(maxmn, 4 * minmn * minmn + 4 * minmn) + maxmn;
				work = new double[lwork];
				Dgesdd.dgesdd("S", m, n, elems, 0, m, s, 0, Ut, 0, m, V, 0, minmn, work, 0, lwork, iwork, 0, info);
			}
		} else {// JOBZ='N'
			lwork = 3 * minmn + Math.max(maxmn, 6 * minmn) + maxmn;
			work = new double[lwork];
			Dgesdd.dgesdd("N", m, n, elems, 0, m, s, 0, null, 0, m, null, 0, n, work, 0, lwork, iwork, 0, info);
		}
	}
	...
	public DoubleMatrix2D getS() {
		...
		if (wantWholeUV == false) {
			S = new DenseDoubleMatrix2D(minmn, minmn);
		} else {
			S = new DenseDoubleMatrix2D(m, n);
		}
	...
	public DoubleMatrix2D getU() {
		if (wantUV == false) {
			...
			if (wantWholeUV == false) {
				return new DenseDoubleMatrix2D(minmn, m).assign(Ut).viewDice().copy();
			} else {
				return new DenseDoubleMatrix2D(m, m).assign(Ut).viewDice().copy();
			}
		}
	}
	...
	public DoubleMatrix2D getUt() {
		if (wantUV == false) {
			...
			if (wantWholeUV == false) {
				return new DenseDoubleMatrix2D(m, minmn).assign(Ut);
			} else {
				return new DenseDoubleMatrix2D(m, m).assign(Ut);
			}
		}
	...
	public DoubleMatrix2D getVt() {
		if (wantUV == false) {
			...
			if (wantWholeUV == false) {
				return new DenseDoubleMatrix2D(minmn, n).assign(V).viewDice().copy();
			} else {
				return new DenseDoubleMatrix2D(n, n).assign(V).viewDice().copy();
			}
		}
	...
	public DoubleMatrix2D getV() {
		if (wantUV == false) {
			...
			if (wantWholeUV == false) {
				return new DenseDoubleMatrix2D(n, minmn).assign(V);
			} else {
				return new DenseDoubleMatrix2D(n, n).assign(V);
			}
		}
	}
