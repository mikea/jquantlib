 ...
 * <p>
 */
public class FloatLUDecompositionQuick implements java.io.Serializable {
	 ...
	 */
	public FloatLUDecompositionQuick(float tolerance) {
		this.algebra = new FloatAlgebra(tolerance);
	}
	...
	public void decompose(FloatMatrix2D A) {
		...
		cern.jet.math.FloatMult multFunction = cern.jet.math.FloatMult.mult(0);
	...
	public void solve(FloatMatrix2D B) {
		...
		// transformations
		cern.jet.math.FloatMult div = cern.jet.math.FloatMult.div(0);
		cern.jet.math.FloatPlusMult minusMult = cern.jet.math.FloatPlusMult.minusMult(0);

		cern.colt.list.IntArrayList nonZeroIndexes = new cern.colt.list.IntArrayList(); // sparsity
		FloatMatrix1D Browk = cern.colt.matrix.FloatFactory1D.dense.make(nx); // blocked
		// row
		// k
		...
		for (int k = n - 1; k >= 0; k--) {
			...
			// blocking
			if (Browk == null)
				Browk = cern.colt.matrix.FloatFactory1D.dense.make(B.columns());
			Browk.assign(Brows[k]);
	...
	public String toString() {
		...
		buf.append("\n\ninverse(A) = ");
		FloatMatrix2D identity = cern.colt.matrix.FloatFactory2D.dense.identity(LU.rows());
		try {
			this.solve(identity);
