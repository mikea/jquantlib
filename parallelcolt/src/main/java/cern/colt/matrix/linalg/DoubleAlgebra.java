...
import cern.colt.matrix.impl.DenseDComplexMatrix1D;
 ...
 */
public class DoubleAlgebra extends cern.colt.PersistentObject {
	...
	public DoubleCholeskyDecomposition chol(DoubleMatrix2D matrix) {
		return new DoubleCholeskyDecomposition(matrix);
	}
	 ...
	 */
	public Object clone() {
		return new DoubleAlgebra(property.tolerance());
	}
	...
	public DoubleEigenvalueDecomposition eig(DoubleMatrix2D matrix) {
		return new DoubleEigenvalueDecomposition(matrix);
	}
	...
	public DoubleLUDecomposition lu(DoubleMatrix2D matrix) {
		return new DoubleLUDecomposition(matrix);
	}

	public DComplexMatrix1D kron(DComplexMatrix1D x, DComplexMatrix1D y) {
		int size_x = x.size();
		int size_y = y.size();
		DComplexMatrix1D C = new DenseDComplexMatrix1D(size_x * size_y);
		for (int i = 0; i < size_x; i++) {
			C.viewPart(i * size_y, size_y).assign(y);
			C.viewPart(i * size_y, size_y).assign(DComplexFunctions.mult(x.getQuick(i)));
		}
		return C;
	}

	public DoubleMatrix1D kron(double[] A, double[] B) {
		int size_a = A.length;
		int size_b = B.length;
		DoubleMatrix1D C = new DenseDoubleMatrix1D(size_a * size_b);
		for (int i = 0; i < size_a; i++) {
			C.viewPart(i * size_b, size_b).assign(B);
			C.viewPart(i * size_b, size_b).assign(DoubleFunctions.mult(A[i]));
		}
		return C;
	}
	...
	public DoubleMatrix1D kron(DoubleMatrix1D x, DoubleMatrix1D y) {
		int size_x = x.size();
		int size_y = y.size();
		DoubleMatrix1D C = new DenseDoubleMatrix1D(size_x * size_y);
		for (int i = 0; i < size_x; i++) {
			C.viewPart(i * size_y, size_y).assign(y);
			C.viewPart(i * size_y, size_y).assign(DoubleFunctions.mult(x.getQuick(i)));
		}
		return C;
	}
	...
	public DoubleMatrix2D multOuter(DoubleMatrix1D x, DoubleMatrix1D y, DoubleMatrix2D A) {
		...
		for (int column = columns; --column >= 0;)
			A.viewColumn(column).assign(x, cern.jet.math.DoubleFunctions.mult);
		return A;
	}
	...
	public double norm1(DoubleMatrix1D x) {
		if (x.size() == 0)
			return 0;
		return x.aggregate(cern.jet.math.DoubleFunctions.plus, cern.jet.math.DoubleFunctions.abs);
	}
	...
	public double vectorNorm2(final DoubleMatrix2D X) {
		if (X.isView() == true || !(X instanceof DenseDoubleMatrix2D)) {
			...
			final double[] elems = (double[]) ((DenseDoubleMatrix2D) X).getElements();
			double sum = 0;
			int np = Utils.getNP();
	...
	public double vectorNorm2(final DoubleMatrix3D X) {
		if (X.isView() == true || !(X instanceof DenseDoubleMatrix3D)) {
			...
			final double[] elems = (double[]) ((DenseDoubleMatrix3D) X).getElements();
			double sum = 0;
			int np = Utils.getNP();
			if ((np > 1) && (elems.length >= cern.colt.Utils.getThreadsBeginN_2D())) {
				Future[] futures = new Future[np];
	...
	public double normF(DoubleMatrix2D A) {
		if (A.size() == 0)
			return 0;
		return A.aggregate(hypotFunction(), cern.jet.math.DoubleFunctions.identity);
	}
	...
	public double normF(DComplexMatrix2D A) {
		...
		for (int r = 0; r < rows; r++) {
			for (int c = 0; c < cols; c++) {
				tmp = DComplex.abs(A.getQuick(r, c));
				norm += tmp * tmp;
			}
		}
	...
	public double normInfinity(DoubleMatrix1D x) {
		...
		if (x.size() == 0)
			return 0;
		return x.aggregate(cern.jet.math.DoubleFunctions.max, cern.jet.math.DoubleFunctions.abs);
	...
	public DoubleQRDecomposition qr(DoubleMatrix2D matrix) {
		return new DoubleQRDecomposition(matrix);
	}
	...
	public DoubleSingularValueDecomposition svd(DoubleMatrix2D matrix) {
		return new DoubleSingularValueDecomposition(matrix);
	}
	...
	public DoubleSingularValueDecompositionDC svdDC(DoubleMatrix2D matrix) {
		return new DoubleSingularValueDecompositionDC(matrix, true, true);
	}
	...
	public String toVerboseString(DoubleMatrix2D matrix) {
		...
		DoubleLUDecomposition lu = null;
		try {
			lu = new DoubleLUDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
		...
		DoubleQRDecomposition qr = null;
		try {
			qr = new DoubleQRDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
		...
		DoubleCholeskyDecomposition chol = null;
		try {
			chol = new DoubleCholeskyDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
		...
		DoubleEigenvalueDecomposition eig = null;
		try {
			eig = new DoubleEigenvalueDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
		...
		DoubleSingularValueDecomposition svd = null;
		try {
			svd = new DoubleSingularValueDecomposition(matrix);
		} catch (IllegalArgumentException exc) {
