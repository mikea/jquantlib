 ...
 * returns <tt>false</tt>.
 */
public class DoubleQRDecomposition implements java.io.Serializable {
	...
	public DoubleQRDecomposition(DoubleMatrix2D A) {
		...
		Rdiag = A.like1D(n);
		// Rdiag = new double[n];
		cern.colt.function.DoubleDoubleFunction hypot = DoubleAlgebra.hypotFunction();
		...
		for (int k = 0; k < n; k++) {
			...
			for (int i = k; i < m; i++) { // fixes bug reported by
				// hong.44@osu.edu
				nrm = DoubleAlgebra.hypot(nrm, QR.getQuick(i, k));
			}
			...
			if (nrm != 0.0) {
				...
				if (QR.getQuick(k, k) < 0)
					nrm = -nrm;
				QRcolumnsPart[k].assign(cern.jet.math.DoubleFunctions.div(nrm));
				/*
				...
				for (int j = k + 1; j < n; j++) {
					...
					/*
					 * // fixes bug reported by John Chambers DoubleMatrix1D
					 * QRcolj = QR.viewColumn(j).viewPart(k,m-k); double s =
					 * QRcolumnsPart[k].zDotProduct(QRcolumns[j]); double s =
	...
	public DoubleMatrix2D getH() {
		return DoubleAlgebra.DEFAULT.trapezoidalLower(QR.copy());
	}
