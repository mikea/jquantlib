 ...
 * returns <tt>false</tt>.
 */
public class FloatQRDecomposition implements java.io.Serializable {
	...
	public FloatQRDecomposition(FloatMatrix2D A) {
		...
		Rdiag = A.like1D(n);
		// Rdiag = new float[n];
		cern.colt.function.FloatFloatFunction hypot = FloatAlgebra.hypotFunction();
		...
		for (int k = 0; k < n; k++) {
			...
			for (int i = k; i < m; i++) { // fixes bug reported by
				// hong.44@osu.edu
				nrm = FloatAlgebra.hypot(nrm, QR.getQuick(i, k));
			}
			...
			if (nrm != 0.0) {
				...
				if (QR.getQuick(k, k) < 0)
					nrm = -nrm;
				QRcolumnsPart[k].assign(cern.jet.math.FloatFunctions.div(nrm));
				/*
	...
	public FloatMatrix2D getH() {
		return FloatAlgebra.DEFAULT.trapezoidalLower(QR.copy());
	}
	...
	public String toString() {
		...
		buf.append("\n\npseudo inverse(A) = ");
		try {
			buf.append(String.valueOf(this.solve(cern.colt.matrix.FloatFactory2D.dense.identity(QR.rows()))));
		} catch (IllegalArgumentException exc) {
			buf.append(unknown + exc.getMessage());
		}
