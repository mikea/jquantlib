...
import cern.colt.matrix.impl.DenseFComplexMatrix2D;
...
public abstract class FComplexMatrix2D extends AbstractMatrix2D {
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public float[] aggregate(final FComplexMatrix2D other, final cern.colt.function.FComplexFComplexFComplexFunction aggr, final cern.colt.function.FComplexFComplexFComplexFunction f) {
		checkShape(other);
		float[] b = new float[2];
		if (size() == 0) {
			b[0] = Float.NaN;
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public FComplexMatrix2D assign(final cern.colt.function.FComplexFComplexFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public FComplexMatrix2D assign(final cern.colt.function.FComplexProcedure cond, final cern.colt.function.FComplexFComplexFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	 ...
	 * 
	 */
	public FComplexMatrix2D assign(final cern.colt.function.FComplexProcedure cond, final float[] value) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public FComplexMatrix2D assign(final cern.colt.function.FComplexRealFunction f) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	 ...
	 * @see cern.jet.math.FComplexFunctions
	 */
	public FComplexMatrix2D assign(final FComplexMatrix2D y, final cern.colt.function.FComplexFComplexFComplexFunction f) {
		checkShape(y);
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
	...
	public FComplexMatrix2D assign(final float[] values) {
		...
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			...
			for (int j = 0; j < np; j++) {
				final int startrow = j * k;
				final int stoprow;
				final int glob_idx = j * k * 2 * columns;
				if (j == np - 1) {
					stoprow = rows;
				} else {
					stoprow = startrow + k;
	 ...
	 * matrix. The real part of the receiver is reset to zero. Both matrices
	 ...
	 * matrix. The imaginary part of the receiver is reset to zero. Both
	...
	public boolean equals(Object obj) {
		...
		return cern.colt.matrix.linalg.FloatProperty.DEFAULT.equals(this, (FComplexMatrix2D) obj);
	}
	 ...
	 */
	public abstract void fftColumns();
	 ...
	 * @return <tt>this</tt> (for convenience only).
	 */
	public FComplexMatrix2D forEachNonZero(final cern.colt.function.IntIntFComplexFunction function) {
		int np = Utils.getNP();
		if ((np > 1) && (size() >= Utils.getThreadsBeginN_2D())) {
			Future[] futures = new Future[np];
			int k = rows / np;
	...
	public void getNonZeros(IntArrayList rowList, IntArrayList columnList, ArrayList<float[]> valueList) {
		...
		for (int row = 0; row < r; row++) {
			for (int column = 0; column < c; column++) {
				float[] value = getQuick(row, column);
				if (value[0] != 0 || value[1] != 0) {
					rowList.add(row);
	 ...
	 *            if true then scaling is performed
	 */
	public abstract void ifftColumns(boolean scale);
	 ...
	 *            if true then scaling is performed
	 */
	public abstract void ifftRows(boolean scale);

	/**
	 * Construct and returns a new empty matrix <i>of the same dynamic type</i>
	 * as the receiver, having the same number of rows and columns. For example,
	 * if the receiver is an instance of type <tt>DenseComplexMatrix2D</tt>
	 * the new matrix must also be of type <tt>DenseComplexMatrix2D</tt>. In
	 ...
	 * <tt>re = values[row][2*column]; im = values[row][2*column+1]</tt> and
	 * has the same number of rows and columns as the receiver.
	 * <p>
	 * The values are copied. So subsequent changes in <tt>values</tt> are not
	 * reflected in the matrix, and vice-versa.
	 * 
				...
				futures[j] = Utils.threadPool.submit(new Runnable() {
					...
					public void run() {
						for (int r = startrow; r < stoprow; r++) {
							...
							zLoc.setQuick(r, FComplex.plus(FComplex.mult(s, alpha), FComplex.mult(zLoc.getQuick(r), beta)));
						}
					}
				});
			}
	...
	public FComplexMatrix2D zMult(FComplexMatrix2D B, FComplexMatrix2D C) {
		return zMult(B, C, new float[] { 1, 0 }, (C == null ? new float[] { 1, 0 } : new float[] { 0, 0 }), false, false);
	}
	...
	public FComplexMatrix2D zMult(final FComplexMatrix2D B, FComplexMatrix2D C, final float[] alpha, final float[] beta, boolean transposeA, boolean transposeB) {
		...
		final FComplexMatrix2D C_loc;
		if (C == null) {
			C_loc = new DenseFComplexMatrix2D(m, p);
		} else {
			C_loc = C;
		}
		if (B.rows != n)
					...
					public void run() {
						for (int a = starta; a < stopa; a++) {
							for (int b = 0; b < m; b++) {
								...
								C_loc.setQuick(b, a, FComplex.plus(FComplex.mult(s, alpha), FComplex.mult(C_loc.getQuick(b, a), beta)));
							}
						}
					}
				});
	...
	public float[] zSum() {
		...
		return aggregate(cern.jet.math.FComplexFunctions.plus, cern.jet.math.FComplexFunctions.identity);
	}
