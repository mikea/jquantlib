...
class ExactFloatQuantileFinder extends cern.colt.PersistentObject implements FloatQuantileFinder {
	 ...
	 */
	public boolean forEach(cern.colt.function.FloatProcedure procedure) {
	...
	public float phi(float element) {
		this.sort();
		return cern.jet.stat.FloatDescriptive.rankInterpolated(buffer, element) / this.size();
	}
	...
	public FloatArrayList quantileElements(FloatArrayList phis) {
		this.sort();
		return cern.jet.stat.FloatDescriptive.quantiles(this.buffer, phis);
		/*
