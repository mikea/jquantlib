...
public class DoubleDescriptive extends Object {
	 ...
	 *            minimum.
	 *            <li><tt>inOut[1] = Double.NEGATIVE_INFINITY</tt> as the old
	 *            maximum.
	 ...
	 * <tt>b(k,i) = </tt>{@link cern.jet.math.DoubleArithmetic#binomial(long,long) binomial(k,i)}
	 * and <br>
	...
	public static double moment(int k, double c, int size, double[] sumOfPowers) {
		...
		for (int i = 0; i <= k; i++) {
			...
			if (i == 0)
				...
				y = Math.pow(c, i);
			// sum += sign *
			sum += sign * cern.jet.math.DoubleArithmetic.binomial(k, i) * y * sumOfPowers[k - i];
			sign = -sign;
		}
		/*
