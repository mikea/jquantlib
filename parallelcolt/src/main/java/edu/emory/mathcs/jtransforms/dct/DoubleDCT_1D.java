...
/**
 * Computes 1D Discrete Cosine Transform (DCT) of double precision data where
...
public class DoubleDCT_1D {
	...
	public DoubleDCT_1D(int n, int[] ip, double[] w) {
		if (!Utils.isPowerOf2(n))
			throw new IllegalArgumentException("Dimension must be power of two");
		this.n = n;
		this.ip = ip;
	...
	public void forward(double[] a, int starta, boolean scale) {
		int j, nw, nc;
		double xr;
		...
		if (scale) {
			scale(a, starta);
			a[starta] = a[starta] / Math.sqrt(2.0);
		}
	}
	...
	public void inverse(double[] a, int starta, boolean scale) {
		int j, nw, nc;
		double xr;
	...
	private void dctsub(int n, double[] a, int starta, int nc, double[] c, int startc) {
		int j, k, kk, ks, m;
		double wkr, wki, xr;
		int idx0, idx1, idx2;
	...
	private void scale(final double[] a, int starta) {
		int np = Utils.getNP();
