 ...
 * precision data where both dimensions are integers power of 2. This is a
...
public class FloatFFT_2D {
	private static final int FFT2D_THREADS_BEGIN_N = 65536;
	 ...
	 * Complex data is represented by 2 float values in sequence: the real and
	 * imaginary parts, i.e. input array must be of size n1*2*n2.
	 * 
	 * @param a
	 *            data to transform
	 */
	public void complexForward(float[] a) {
		...
		if ((np > 1) && (n1 * n2 >= FFT2D_THREADS_BEGIN_N)) {
			xdft2d0_subth(0, -1, a, true);
	 ...
	 * i.e. input array must be of size n1 by 2*n2.
	 * 
	 * @param a
	 *            data to transform
	 */
	...
	public void realForwardFull(float[][] a) {
		...
		if ((np > 1) && ((float) n1 * n2 >= (float) FFT2D_THREADS_BEGIN_N)) {
			fillSymmetric(a);
		} else {
			for (int k1 = 1; k1 < n1d2; k1++) {
				a[k1][n2] = a[n1 - k1][1];
	...
	private void xdft2d0_subth(final int icr, final int isgn, final float[] a, final boolean scale) {
		final int nthread;
		int i;
		...
		for (int i = 0; i < np; i++) {
			...
			futures[i] = Utils.threadPool.submit(new Runnable() {
				public void run() {
					...
					for (int k1 = l1starta; k1 < l1stopa; k1++) {
						idx1 = k1 * newn2;
						idx2 = (n1 - k1) * newn2;
						a[idx1 + n2] = a[idx2 + 1];
						a[idx1 + n2 + 1] = -a[idx2];
					...
					for (int k1 = l2starta; k1 < l2stopa; k1++) {
						for (int k2 = 0; k2 < newn2; k2 = k2 + 2) {
