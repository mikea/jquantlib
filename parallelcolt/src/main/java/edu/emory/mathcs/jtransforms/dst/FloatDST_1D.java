...
/**
 * Computes 1D Discrete Sine Transform (DST) of single precision data where
...
public class FloatDST_1D {
	...
	protected FloatDST_1D(int n, int[] ip, float[] w) {
		this.n = n;
	...
	protected void forward(final float[] a, final int starta, boolean scale) {
		float tmp;
		int np = Utils.getNP();
	...
	protected void inverse(final float[] a, final int starta, boolean scale) {
		float tmp;
		int np = Utils.getNP();
		if ((np > 1) && (n > DST1D_THREADS_BEGIN_N)) {
			final int k = n / 2 / np;
