 ...
 * precision data where dimension is an integer power of 2. This is a parallel
...
public class DoubleFFT_1D {
	private static final long FFT1D_2THREADS_BEGIN_N = 8192;

	private static final long FFT1D_4THREADS_BEGIN_N = 65536;
	...
	protected DoubleFFT_1D(int n, int[] ip, double[] w) {
	 ...
	 * <code>a</code>. Complex data is represented by 2 double values in
	 * sequence: the real and imaginary parts, i.e. input array must be of size
	 * 2*n.
	 * 
	 * @param a
	 *            data to transform
	...
	protected void complexForward(double[] a, int starta) {
		int nw;
		int twon = 2 * n;
		nw = ip[0];
		if (twon > (nw << 2)) {
	...
	protected void complexInverse(double[] a, int starta, boolean scale) {
		int nw;
		int twon = 2 * n;
		nw = ip[0];
		if (twon > (nw << 2)) {
		...
		cftfsub(twon, a, starta, ip, nw, w);
		if (scale) {
			scale(n, a, starta, true);
		}
	}

	/**
	 * Computes 1D forward DFT of real data leaving the result in <code>a</code>.
	 * The physical layout in the output data is as follows:
	 * 
	 * <pre>
	 * a[2*k] = Re[k], 0&lt;=k&lt;n/2
	 ...
	 * </pre>
	 * 
	 * This method computes only half of the elements of real transform. The
	 * other half satisfy the symmetry condition. If you want the full real
	 * forward transform, use <code>realForwardFull</code>.
	 * 
	 * @param a
	 *            data to transform
	 */
	...
	protected void realForward(double[] a, int starta) {
		int nw, nc;
		double xi;
	 ...
	 * the input array must be of size 2*n, with only the first n elements
	...
	protected void realForwardFull(double[] a, int starta) {
		int nw, nc;
		double xi;
		int twon = 2 * n;
		nw = ip[0];
		...
		for (int k = 0; k < twon; k = k + 2) {
			a[(twon - k) % twon] = a[k];
			a[(twon - k) % twon + 1] = -a[k + 1];
		}
		a[n] = -a[1];
		a[1] = 0;
	}
	...
	protected void realInverse(double[] a, int starta, boolean scale) {
		int nw, nc;
		double xi;
	...
	protected void realInverseFull(double[] a, int starta, boolean scale) {
		int nw, nc;
		double xi;
		int twon = 2 * n;
		nw = ip[0];
	...
	private void cftfsub(int n, double[] a, int starta, int[] ip, int nw, double[] w) {
		if (n > 8) {
			if (n > 32) {
				cftf1st(n, a, starta, w, nw - (n >> 2));
				if (n > FFT1D_2THREADS_BEGIN_N) {
	...
	private void cftbsub(int n, double[] a, int starta, int[] ip, int nw, double[] w) {
		if (n > 8) {
			if (n > 32) {
	...
	private void bitrv2(int n, int[] ip, double[] a, int starta) {
	...
	private void bitrv2conj(int n, int[] ip, double[] a, int starta) {
		int j, j1, k, k1, l, m, nh, nm;
		double xr, xi, yr, yi;
		int idx1, idx2;
	...
	private void bitrv216(double[] a, int starta) {
	...
	private void bitrv216neg(double[] a, int starta) {
	...
	private void bitrv208(double[] a, int starta) {
	...
	private void bitrv208neg(double[] a, int starta) {
	...
	private void cftf1st(int n, double[] a, int starta, double[] w, int startw) {
	...
	private void cftb1st(int n, double[] a, int starta, double[] w, int startw) {
		int j, j0, j1, j2, j3, k, m, mh;
		double wn4r, csc1, csc3, wk1r, wk1i, wk3r, wk3i, wd1r, wd1i, wd3r, wd3i;
		double x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i, y0r, y0i, y1r, y1i, y2r, y2i, y3r, y3i;
		int idx0, idx1, idx2, idx3, idx4, idx5;
	...
	private void cftrec4_th(final int n, final double[] a, final int starta, final int nw, final double[] w) {
		int i;
		int idiv4, m, nthread;
		int idx = 0;
		nthread = 2;
	...
	private void cftrec4(int n, double[] a, int starta, int nw, double[] w) {
		int isplt, j, k, m;
	...
	private int cfttree(int n, int j, int k, double[] a, int starta, int nw, double[] w) {
		int i, isplt, m;
		int idx1 = starta - n;
		if ((k & 3) != 0) {
			isplt = k & 1;
	...
	private void cftleaf(int n, int isplt, double[] a, int starta, int nw, double[] w) {
		if (n == 512) {
	...
	private void cftmdl1(int n, double[] a, int starta, double[] w, int startw) {
		int j, j0, j1, j2, j3, k, m, mh;
	...
	private void cftmdl2(int n, double[] a, int starta, double[] w, int startw) {
	...
	private void cftfx41(int n, double[] a, int starta, int nw, double[] w) {
		if (n == 128) {
			cftf161(a, starta, w, nw - 8);
			cftf162(a, starta + 32, w, nw - 32);
			cftf161(a, starta + 64, w, nw - 8);
	...
	private void cftf161(double[] a, int starta, double[] w, int startw) {
	...
	private void cftf162(double[] a, int starta, double[] w, int startw) {
	...
	private void cftf081(double[] a, int starta, double[] w, int startw) {
	...
	private void cftf082(double[] a, int starta, double[] w, int startw) {
	...
	private void cftf040(double[] a, int starta) {
		double x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
	...
	private void cftb040(double[] a, int starta) {
		double x0r, x0i, x1r, x1i, x2r, x2i, x3r, x3i;
	...
	private void cftx020(double[] a, int starta) {
		double x0r, x0i;
	...
	private void cftxb020(double[] a, int starta) {
		double x0r, x0i;
	...
	private void rftfsub(int n, double[] a, int starta, int nc, double[] c, int startc) {
		int j, k, kk, ks, m;
		...
		a[starta + m + 1] = -a[starta + m + 1];
	}

	private void rftbsub(int n, double[] a, int starta, int nc, double[] c, int startc) {
		int j, k, kk, ks, m;
		double wkr, wki, xr, xi, yr, yi;
		int idx1, idx2;
	...
	private void scale(final int m, final double[] a, int starta, boolean complex) {
		int locn;
		if (complex) {
			locn = 2 * n;
		} else {
		...
		int np = Utils.getNP();
		if ((np > 1) && (locn > FFT1D_2THREADS_BEGIN_N)) {
			final int k = locn / np;
			Future[] futures = new Future[np];
			for (int i = 0; i < np; i++) {
				final int idx1 = starta + i * k;
			...
			for (int i = starta; i < starta + locn; i++) {
				a[i] = a[i] / m;
			}
		}
	}
