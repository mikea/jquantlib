...
public class DoubleFFT_2D {
	...
	public DoubleFFT_2D(int n1, int n2) {
		...
		this.w = new double[(int) Math.ceil(Math.max(Math.max(n1 / 2, n2 / 2), Math.max(n1 / 2, n2 / 4) + n2 / 4))];
	...
	public void realForwardFull(double[][] a) {
		...
		if ((np > 1) && ((double) n1 * n2 >= (double) FFT2D_THREADS_BEGIN_N)) {
			fillSymmetric(a);
		} else {
			for (int k1 = 1; k1 < n1d2; k1++) {
				a[k1][n2] = a[n1 - k1][1];
	...
	private void xdft2d0_subth(final int icr, final int isgn, final double[] a, final boolean scale) {
		final int nthread;
		int i;
