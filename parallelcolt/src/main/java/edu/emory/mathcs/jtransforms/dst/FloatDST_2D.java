...
/**
 * Computes 2D Discrete Sine Transform (DST) of single precision data where both
 * dimensions are integers power of 2. This is a parallel implementation of
...
public class FloatDST_2D {
	...
	private static final int DST2D_THREADS_BEGIN_N = 65536;
	...
	private FloatDST_1D dstn2, dstn1;
	 ...
	 */
	public FloatDST_2D(int n1, int n2) {
		...
		dstn2 = new FloatDST_1D(n2, ip, w);
		dstn1 = new FloatDST_1D(n1, ip, w);
		this.np = Utils.getNP();
		;
		nt = 4 * np * n1;
		if (n2 == 2 * np) {
	...
	public void forward(float[] a, boolean scale) {
		...
		if ((np > 1) && (n1 * n2 >= DST2D_THREADS_BEGIN_N)) {
