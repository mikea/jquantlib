 ...
 * precision data where all three dimensions are integers power of 2. This is a
 * parallel implementation of split-radix algorithm optimized for SMP systems.
 * <br>
 * <br>
...
public class FloatFFT_3D {
	private static final int FFT3D_THREADS_BEGIN_N = 65536;
	...
	private FloatFFT_1D fftn1, fftn2, fftn3;
	 ...
	 * 
	 */
	public FloatFFT_3D(int n1, int n2, int n3) {
		...
		w = new float[(int) Math.ceil(Math.max(Math.max(Math.max(n1 / 2, n2 / 2), n3 / 2), Math.max(Math.max(n1 / 2, n2 / 2), n3 / 4) + n3 / 4))];
		nt = (Math.max(8 * n1, 8 * n2)) * np;
		if (n3 == 4) {
			nt >>= 1;
		} else if (n3 < 4) {
			nt >>= 2;
	 ...
	 * i.e. element (i,j,k) of 3D array x[n1][n2][n3] is stored in
	 * a[i*sliceStride + j*rowStride + k], where sliceStride = n2 * n3 and
	 * rowStride = n3. Complex data is represented by 2 float values in
	...
	public void complexForward(float[] a) {
		...
		if ((np > 1) && (n1 * n2 * n3 >= FFT3D_THREADS_BEGIN_N)) {
			xdft3da_subth(0, -1, a, true);
			cdft3db_subth(-1, a, true);
		} else {
			xdft3da_sub(0, -1, a, true);
	 ...
	 * i.e. input array must be of size n1 by n2 by 2*n3.
	 * 
	 * @param a
	 *            data to transform
	 */
	 ...
	 * slice-major, then row-major, then column-major, in order of significance,
	 * i.e. element (i,j,k) of 3-d array x[n1][n2][n3] is stored in
	 * a[i*sliceStride + j*rowStride + k], where sliceStride = n2 * n3 and
	 * rowStride = n3. Complex data is represented by 2 float values in
	 * sequence: the real and imaginary parts, i.e. input array must be of size
	 * n1*n2*2*n3.
	...
	public void complexInverse(float[] a, boolean scale) {
		...
		if ((np > 1) && (n1 * n2 * n3 >= FFT3D_THREADS_BEGIN_N)) {
			xdft3da_subth(0, 1, a, scale);
			cdft3db_subth(1, a, scale);
		} else {
			xdft3da_sub(0, 1, a, scale);
	 ...
	 * (i,j,k) of 3-d array x[n1][n2][n3] is stored in a[i*sliceStride +
	 * j*rowStride + k], where sliceStride = n2 * n3 and rowStride = n3. The
	 * physical layout in the output data is as follows:
	 * 
	 * <pre>
	...
	public void realForward(float[] a) {
		...
		if ((np > 1) && (n1 * n2 * n3 >= FFT3D_THREADS_BEGIN_N)) {
			xdft3da_subth(1, -1, a, true);
			cdft3db_subth(-1, a, true);
			rdft3d_sub(1, a);
		} else {
			xdft3da_sub(1, -1, a, true);
			cdft3db_sub(-1, a, true);
			rdft3d_sub(1, a);
		}
	}
	...
	/**
	 * Computes 3D forward DFT of real data leaving the result in <code>a</code>.
	 * The data is stored in a 3D array. The physical layout in the output data
	 * is as follows:
	 * 
	 * <pre>
	 * a[k1][k2][2*k3] = Re[k1][k2][k3]
	...
	public void realForwardFull(float[] a) {
		...
		for (k1 = (n1 - 1); k1 >= 1; k1--) {
			for (k2 = 0; k2 < n2; k2++) {
				for (k3 = 0; k3 < n3; k3++) {
					...
					a[k1 * newSliceStride + k2 * newRowStride + k3] = a[idx1];
					a[idx1] = 0;
				}
			}
		}
		...
		for (k2 = 1; k2 < n2; k2++) {
			for (k3 = 0; k3 < n3; k3++) {
				...
				a[(n2 - k2) * newRowStride + k3] = a[idx1];
				a[idx1] = 0;
			}
		}
	...
	public void realForwardFull(float[][][] a) {
		...
		if ((np > 1) && ((float) n1 * n2 * n3 >= (float) FFT3D_THREADS_BEGIN_N)) {
			fillSymmetric(a);
		} else {
	...
	public void realInverse(float[] a, boolean scale) {
		...
		if ((np > 1) && (n1 * n2 * n3 >= FFT3D_THREADS_BEGIN_N)) {
			rdft3d_sub(-1, a);
			cdft3db_subth(1, a, scale);
			xdft3da_subth(1, -1, a, scale);
		} else {
			xdft3da_sub(1, -1, a, scale);
			cdft3db_sub(1, a, scale);
			rdft3d_sub(1, a);
	...
	private void xdft3da_sub(int icr, int isgn, float[] a, boolean scale) {
	...
	private void xdft3da_subth(final int icr, final int isgn, final float[] a, final boolean scale) {
		final int nthread;
		int nt, i;
	...
	private void fillSymmetric(final float[] a) {
		...
		for (int i = 0; i < np; i++) {
			final int l1starta, l1stopa, l2starta, l2stopa;
			l1starta = i * l1k;
			l1stopa = l1starta + l1k;
			if (i == 0) {
				l2starta = i * l2k + 1;
			} else {
				l2starta = i * l2k;
			...
			futures[i] = Utils.threadPool.submit(new Runnable() {
				...
				public void run() {
					int k1, k2, k3, idx1, idx2, idx3, idx4;
					// -----------------------------------------------
					for (k1 = l1starta; k1 < l1stopa; k1++) {
						for (k2 = 0; k2 < n2; k2++) {
							for (k3 = 1; k3 < n3; k3 = k3 + 2) {
								idx1 = ((n1 - k1) % n1) * newSliceStride + ((n2 - k2) % n2) * newRowStride + newn3 - k3;
								idx2 = k1 * newSliceStride + k2 * newRowStride + k3;
					...
					for (k1 = l2starta; k1 < l2stopa; k1++) {

						idx1 = k1 * newSliceStride;
						idx2 = (n1 - k1) * newSliceStride;
						idx4 = k1 * newSliceStride + n2d2 * newRowStride;
						idx3 = (n1 - k1) * newSliceStride + n2d2 * newRowStride;
						a[idx1 + n3] = a[idx2 + 1];
						a[idx2 + n3] = a[idx2 + 1];
						a[idx1 + n3 + 1] = -a[idx2];
						a[idx2 + n3 + 1] = a[idx2];
