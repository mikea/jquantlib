...
/**
 * Computes 3D Discrete Sine Transform (DST) of single precision data where all
 * three dimensions are integers power of 2. This is a parallel implementation
 * of split-radix algorithm optimized for SMP systems. <br>
 * <br>
...
public class FloatDST_3D {
	...
	private static final int DST3D_THREADS_BEGIN_N = 65536;
	...
	private FloatDCT_1D dstn1, dstn2, dstn3;
	 ...
	 */
	public FloatDST_3D(int n1, int n2, int n3) {
		...
		dstn1 = new FloatDCT_1D(n1, ip, w);
		dstn2 = new FloatDCT_1D(n2, ip, w);
		dstn3 = new FloatDCT_1D(n3, ip, w);
		this.np = Utils.getNP();
		if (np > 1) {
	...
	public void forward(float[] a, boolean scale) {
		...
		t = new float[nt];
		if ((np > 1) && (n1 * n2 * n3 >= DST3D_THREADS_BEGIN_N)) {
