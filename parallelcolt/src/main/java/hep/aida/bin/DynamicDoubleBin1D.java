...
import cern.jet.stat.DoubleDescriptive;
 ...
 * use {@link StaticDoubleBin1D}, which overcomes them at the expense of
 ...
 * check out {@link cern.jet.stat.DoubleDescriptive} in combination with methods
 * {@link #elements()} and {@link #sortedElements()}.
 * <p>
 * <b>Implementation</b>: Lazy evaluation, caching, incremental maintainance.
 * 
 * @see cern.jet.stat.DoubleDescriptive
...
public class DynamicDoubleBin1D extends QuantileDoubleBin1D {
	 ...
	 */
	public synchronized void sample(int n, boolean withReplacement, DoubleRandomEngine randomGenerator, cern.colt.buffer.DoubleBuffer buffer) {
		if (randomGenerator == null)
			randomGenerator = cern.jet.random.DoubleUniform.makeDefaultGenerator();
		buffer.clear();

		if (!withReplacement) { // without
			if (n > size())
				throw new IllegalArgumentException("n must be less than or equal to size()");
			cern.jet.random.sampling.DoubleRandomSamplingAssistant sampler = new cern.jet.random.sampling.DoubleRandomSamplingAssistant(n, size(), randomGenerator);
			for (int i = n; --i >= 0;) {
				if (sampler.sampleNextElement())
					buffer.add(this.elements.getQuick(i));
			}
		} else { // with
			cern.jet.random.DoubleUniform uniform = new cern.jet.random.DoubleUniform(randomGenerator);
			int s = size();
			for (int i = n; --i >= 0;) {
	 ...
	 */
	public synchronized DynamicDoubleBin1D sampleBootstrap(DynamicDoubleBin1D other, int resamples, cern.jet.random.engine.DoubleRandomEngine randomGenerator, DoubleBinBinFunction1D function) {
		if (randomGenerator == null)
			randomGenerator = cern.jet.random.DoubleUniform.makeDefaultGenerator();
