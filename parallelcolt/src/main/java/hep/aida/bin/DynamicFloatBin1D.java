...
import cern.jet.stat.FloatDescriptive;
 ...
 * check out {@link cern.jet.stat.FloatDescriptive} in combination with methods
 * {@link #elements()} and {@link #sortedElements()}.
 * <p>
 * <b>Implementation</b>: Lazy evaluation, caching, incremental maintainance.
 * 
 * @see cern.jet.stat.FloatDescriptive
...
public class DynamicFloatBin1D extends QuantileFloatBin1D {
	 ...
	 */
	public synchronized float aggregate(cern.colt.function.FloatFloatFunction aggr, cern.colt.function.FloatFunction f) {
		int s = size();
		if (s == 0)
			return Float.NaN;
	 ...
	 */
	private synchronized cern.colt.map.AbstractFloatIntMap frequencyMap() {
		// cern.colt.map.OpenFloatIntHashMap.hashCollisions = 0;
		// fill a map that collects frequencies
		cern.colt.map.AbstractFloatIntMap map = new cern.colt.map.OpenFloatIntHashMap();
	 ...
	 */
	public synchronized void sample(int n, boolean withReplacement, FloatRandomEngine randomGenerator, cern.colt.buffer.FloatBuffer buffer) {
		if (randomGenerator == null)
			randomGenerator = cern.jet.random.FloatUniform.makeDefaultGenerator();
		buffer.clear();

		if (!withReplacement) { // without
			if (n > size())
				throw new IllegalArgumentException("n must be less than or equal to size()");
			cern.jet.random.sampling.FloatRandomSamplingAssistant sampler = new cern.jet.random.sampling.FloatRandomSamplingAssistant(n, size(), randomGenerator);
			for (int i = n; --i >= 0;) {
				if (sampler.sampleNextElement())
					buffer.add(this.elements.getQuick(i));
			}
		} else { // with
			cern.jet.random.FloatUniform uniform = new cern.jet.random.FloatUniform(randomGenerator);
			int s = size();
			for (int i = n; --i >= 0;) {
	 ...
	 */
	public synchronized DynamicFloatBin1D sampleBootstrap(DynamicFloatBin1D other, int resamples, cern.jet.random.engine.FloatRandomEngine randomGenerator, FloatBinBinFunction1D function) {
		if (randomGenerator == null)
			randomGenerator = cern.jet.random.FloatUniform.makeDefaultGenerator();
		...
		cern.colt.buffer.FloatBuffer buffer1 = sample1.buffered(Math.min(maxCapacity, s1));

		DynamicFloatBin1D sample2 = new DynamicFloatBin1D();
		cern.colt.buffer.FloatBuffer buffer2 = sample2.buffered(Math.min(maxCapacity, s2));

		DynamicFloatBin1D bootstrap = new DynamicFloatBin1D();
		cern.colt.buffer.FloatBuffer bootBuffer = bootstrap.buffered(Math.min(maxCapacity, resamples));
