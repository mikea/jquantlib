...
The impatient may prefer to jump directly to ''Building JQuantLib''
...
instrument valuation. You also need to be confortable to use Linux.
...
This README file is directly extracted from one article of our wiki pages
which can be accessed at [http://www.jquantlib.org/ JQuantLib website].
This is the reason why this text contains some a little bit weird
formatting in it when visualised in a text editor instead from the
website.
...
In spite Debian offers Maven as a installable package, we prefer to install
it by hand because it's a better controlled method which does not imply on
installation of the entire Java environment chosen by Debian packages. Doing
so, you control which Java environment you are using and in the future, you
will be able to switch to Java7 in order to take advantage of some advanced
compilation options it offers.
...
'''REVIEW DOWNLOAD LOCATIONS
 ...
 wget http://maven.apache.org/apache-maven-2.0.9-bin.tar.bz2
 wget http://maven.apache.org/maven-ant-tasks-2.0.9.jar
...
'''REVIEW PACKAGES
 ...
 apt-get install graphwiz gnuplot latex
...
its build process. We also present here all plugins we recommend to be
...
===Install plugins===
We assume you know how plugins can be installed in Eclipse via update sites.
...
FindBugs and additionaly analyses complexity of your code. Maven offers
...
The following plugins are under evaluation and maybe adopted in the future.
...
Copyright (c) 2008, JQuantLib.org
