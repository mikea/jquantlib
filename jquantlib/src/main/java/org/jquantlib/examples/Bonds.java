...
public class Bonds {
    ...
    private void runBondExample() {
        ...
        // FIXME: outdated...
        Date settlementDate = DateFactory.getFactory().getDate(18, Month.SEPTEMBER, 2008);
        ...
        /* @Rate */double zc3mQuote = 0.0096;
        /* @Rate */double zc6mQuote = 0.0145;
        /* @Rate */double zc1yQuote = 0.0194;

        Quote zc3mRate = (new SimpleQuote(zc3mQuote));
        Quote zc6mRate = (new SimpleQuote(zc6mQuote));
        Quote zc1yRate = (new SimpleQuote(zc1yQuote));

        DayCounter zcBondsDayCounter = Actual365Fixed.getDayCounter();

        RateHelper zc3m = new DepositRateHelper(new Handle<Quote>(zc3mRate), new Period(3, TimeUnit.MONTHS), fixingDays, calendar,
                BusinessDayConvention.MODIFIED_FOLLOWING, true, zcBondsDayCounter);
        RateHelper zc6m = new DepositRateHelper(new Handle<Quote>(zc6mRate), new Period(6, TimeUnit.MONTHS), fixingDays, calendar,
                BusinessDayConvention.MODIFIED_FOLLOWING, true, zcBondsDayCounter);
        RateHelper zc1y = new DepositRateHelper(new Handle<Quote>(zc1yRate), new Period(1, TimeUnit.YEARS), fixingDays, calendar,
                BusinessDayConvention.MODIFIED_FOLLOWING, true, zcBondsDayCounter);
        ...
        Date issueDates[] = { DateFactory.getFactory().getDate(15, Month.MARCH, 2005),
                DateFactory.getFactory().getDate(15, Month.JUNE, 2005), DateFactory.getFactory().getDate(30, Month.JUNE, 2006),
                DateFactory.getFactory().getDate(15, Month.NOVEMBER, 2002), DateFactory.getFactory().getDate(15, Month.MAY, 1987) };

        Date maturities[] = { DateFactory.getFactory().getDate(31, Month.AUGUST, 2010),
                DateFactory.getFactory().getDate(31, Month.AUGUST, 2011), DateFactory.getFactory().getDate(31, Month.AUGUST, 2013),
                DateFactory.getFactory().getDate(15, Month.AUGUST, 2018), DateFactory.getFactory().getDate(15, Month.MAY, 2038) };

        /* @Real */double couponRates[] = { 0.02375, 0.04625, 0.03125, 0.04000, 0.04500 };

        /* @Real */double marketQuotes[] = { 100.390625, 106.21875, 100.59375, 101.6875, 102.140625 };

        List<SimpleQuote> quote = new LinkedList<SimpleQuote>();
