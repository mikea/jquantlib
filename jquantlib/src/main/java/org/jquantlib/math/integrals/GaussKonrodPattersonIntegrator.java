...
public class GaussKonrodPattersonIntegrator implements Integrator {
	...
	public GaussKonrodPattersonIntegrator(double relativeAccuracy, double absoluteAccuracy){
	...
	public double integrate(UnaryFunctionDouble f, double a, double b) {
	    ...
	    for (int k = 0; k < 5; k++)
	        ...
	        //Change from QNG...
	        if (err < getAbsoluteAccuracy() || err < getRelativeAccuracy() * Math.abs(result)){
	        ...
	        for (int k = 0; k < 11; k++){
	            ...
	            double fval = (f.evaluate(center + abscissa)
	                + f.evaluate(center - abscissa));
	            res43 += fval * w43b[k];
	            savfun[k + 10] = fval;
	        }
	       ...
	       if (err < getAbsoluteAccuracy() || err < getRelativeAccuracy() * Math.abs(result)){
	        ...
	        for (int k = 0; k < 22; k++){
	            ...
	            res87 += w87b[k] * (f.evaluate(center + abscissa)
	                + f.evaluate(center - abscissa));
	        }
