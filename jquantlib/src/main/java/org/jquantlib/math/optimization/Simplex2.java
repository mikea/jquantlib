...
/* This is a re-implementation of Simplex, from the original C++ source.  Currently,
 * the junit test for the SABR implementation fails, *unless* Simplex.java is 
...
public class Simplex2 extends OptimizationMethod {
	/* !ructor taking as input the characteristic length */
	public Simplex2(double lambda) {
		lambda_ = lambda;
	}
	...
	double computeSimplexSize(List<Array> vertices) {
		Array center = new Array(vertices.get(0).size(), 0.0);
	...
	double extrapolate(Problem P, int iHighest, double factor) {
		...
		do {
			...
			factor *= 0.5;
		} while (!P.constraint().test(pTry) && Math.abs(factor) > DBL_EPSILON); // QL_EPSILON);
		if (Math.abs(factor) <= DBL_EPSILON) {
	...
	public EndCriteria.Type minimize(Problem P, EndCriteria endCriteria) {
		...
		double xtol = endCriteria.getRootEpsilon(); // end criteria on x (see
													// GSL v. 1.9,
