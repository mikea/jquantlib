 ...
 */
// TODO: Finish
public abstract class  AbstractBinomialVanillaEngine<T extends Tree> extends VanillaOptionEngine{

	private GeneralizedBlackScholesProcess process_;
	private int timeSteps_;
	
	public AbstractBinomialVanillaEngine(GeneralizedBlackScholesProcess process,
			int timeSteps) {
	...
	public void calculate() {
		...
		Handle<YieldTermStructure> flatRiskFree = new Handle<YieldTermStructure>(
		...
		StochasticProcess1D bs = new GeneralizedBlackScholesProcess(process_
				.stateVariable(), flatDividends, flatRiskFree, flatVol);

		TimeGrid grid = new TimeGrid(maturity, timeSteps_);

		Tree tree = getTreeInstance(bs, maturity, timeSteps_, payoff.getStrike());

		BlackScholesLattice<Tree> lattice = new BlackScholesLattice<Tree>(tree, r,
				maturity, timeSteps_);

		DiscretizedVanillaOption option = new DiscretizedVanillaOption(
				arguments, process_, grid);
		...
		results.theta = Greeks.blackScholesTheta(process_, results.value,
				results.delta, results.gamma);

	}

	private Tree getTreeInstance(StochasticProcess1D bs, double maturity,
			int timeSteps, double strike) {
		try {
			
			final Class rsgClass =  TypeToken.getClazz(this
					.getClass());
			final Constructor c = rsgClass.getConstructor(
					StochasticProcess1D.class, double.class, int.class,
					double.class);
