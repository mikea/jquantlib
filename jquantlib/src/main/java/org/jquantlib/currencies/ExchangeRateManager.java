...
public class ExchangeRateManager {
     ...
     */
    public ExchangeRate lookup(final Currency source, final Currency target, Date date, ExchangeRate.Type type) {
     ...
     * @return
     */
    public boolean hashes(/* ExchangeRateManager::Key */int k, final Currency c) {
    ...
    private ExchangeRate directLookup(final Currency source, final Currency target, final Date date) {
        ...
        assert ((rate = fetch(source, target, date)) != null) : "no direct conversion available from " + source.code() + " to "
                + target.code() + " for " + date.getShortFormat();
    ...
    private ExchangeRate smartLookup(final Currency source, final Currency target, final Date date, int[] forbidden) {
        ...
        throw new AssertionError("no conversion available from " + source.code() + " to " + target.code() + " for "
                + date.getShortFormat());
    }
