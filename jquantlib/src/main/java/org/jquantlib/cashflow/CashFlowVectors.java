...
public class CashFlowVectors {
    ...
    public static class Detail {
        ...
        public static <T extends Number,U extends Number> T get(ArrayList<T>  v,
              int i,
              U defaultValue) {
            if (v == null || v.size() == 0) {
        ...
        public static /*Rate*/ double effectiveFixedRate(final Array spreads,
                                final Array caps,
                                final Array floors,
                                /*Size*/int i){
    ...
    public static class FloatingLeg<InterestRateIndexType,
                                    FloatingCouponType,
                                    CappedFlooredCouponType> extends CashFlowVectors.Leg{
        
        
        public FloatingLeg( final Array nominals, 
      ...
      QL_REQUIRE(nominals.length <= n,
                 "too many nominals (" + nominals.length +
                 "), only " + n + " required");
      QL_REQUIRE(gearings.length<=n,
                 "too many gearings (" + gearings.length +
                 "), only " + n + " required");
      QL_REQUIRE(spreads.length<=n,
                 "too many spreads (" + spreads.length+
                 "), only " + n + " required");
      QL_REQUIRE(caps.length<=n,
                 "too many caps (" + caps.length +
                 "), only " + n + " required");
      QL_REQUIRE(floors.length<=n,
                 "too many floors (" + floors.length +
                 "), only " + n + " required");
