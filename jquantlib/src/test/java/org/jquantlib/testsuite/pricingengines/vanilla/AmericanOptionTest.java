...
public class AmericanOptionTest {
    ...
    public void testBjerksundStenslandValues() {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            Date exDate = DateFactory.getFactory().getDate(today.getDayOfMonth(), today.getMonthEnum(), today.getYear()).increment(
                    daysToExpiry);
            ...
            StochasticProcess stochProcess = new BlackScholesMertonProcess(new Handle<Quote>(spot), new Handle<YieldTermStructure>(
                    qTS), new Handle<YieldTermStructure>(rTS), new Handle<BlackVolTermStructure>(volTS));

            VanillaOption option = new VanillaOption(stochProcess, payoff, exercise, engine);
    ...
    /*
     *  Support Data Holder inner classes ...
     */
