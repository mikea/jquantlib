...
/**
 * A simple checker that treats the {@code \@CompoundFactor} annotation as a
 * subtype-style qualifier with no special semantics.
 */
@SupportedSourceVersion(SourceVersion.RELEASE_7)
public class CompoundFactorChecker extends BaseTypeChecker {

    private SimpleSubtypeRelation relation;

    private AnnotationFactory annoFactory;

    /** Represents the {@code \@CompoundFactor} annotation. */
    ...
    public synchronized void init(ProcessingEnvironment processingEnv) {
        ...
        annMirror = this.annoFactory.fromName(CompoundFactor.class.getCanonicalName());
